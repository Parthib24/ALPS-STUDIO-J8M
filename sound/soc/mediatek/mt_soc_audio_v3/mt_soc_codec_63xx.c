...
#else
static int Speaker_mode = AUDIO_SPEAKER_MODE_D;//AUDIO_SPEAKER_MODE_AB;		//RGK use ClassD internal audio pa as default.
#endif
...
//Add by daihongyi for cy custom Start
extern volatile int speech_md_usage_control;  //1:incall state  0:media state 
//Add by daihongyi for cy custom End
...
//+++++rgk lihua 2015-01-30  start    
    if (enable)
    {
        ...
        Sound_Speaker_Turnon(speech_md_usage_control);
    }
    else
    {
...
#ifdef CONFIG_RGK_HW_BOM_COMPATIBLE
extern int get_internal_pa_type(void);
#endif
...
#ifdef CONFIG_RGK_HW_BOM_COMPATIBLE	
	pr_warn("%s: before init Speaker_mode\n", __func__);
	if(Speaker_mode == -1){
		Speaker_mode = get_internal_pa_type();
		pr_warn("%s: pa mode is %d:\n", __func__, Speaker_mode);
	}
	pr_warn("%s: after init Speaker_mode\n", __func__);
#endif
