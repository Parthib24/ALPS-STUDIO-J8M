...
#include "msg22xx_hlt_v16_update_bin.h" // for MSG22xx   //helitai
...
//	extern int  fix_tp_proc_info(void  *tp_data, unsigned char data_len);
	...
	if(pMajor==0xc){
		len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","JUNDA",0x26,pMajor,pMinor,12,1);
	}else{
