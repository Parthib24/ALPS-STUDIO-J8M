...
static ssize_t gtp_gesture_data_read(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	if(gesture_data.data[0] == 0xcc)
			 sprintf(buff,"%s\n","f");
ret = simple_read_from_buffer(page, size, ppos, buff, strlen(buff));
	GTP_DEBUG("sym Got the gesture data %x.gesture_data.enabled=%x,*page=%x\n",gesture_data.data[0],gesture_data.enabled,*page);
                  return ret;
}
...
static ssize_t gtp_gesture_data_write(struct file *filp, const char __user * buff, size_t len, loff_t * off)
{
	...
	int j=0;
	int k = 0;
	s32 ret = copy_from_user(temp, buff, 3);

	if (ret) {
		printk("copy_from_user failed.");
		return -EPERM;
	}
	//printk("sym gtp_gesture_data_write %x,%x,%x,%x\n",temp[0],temp[1],temp[2],temp[3]);


	if(( temp[0]!='c' ) || ( temp[1]!='c') || (temp[2]!='2' && temp[2]!='1' ) )
	{
		printk("sym gesture bad command format 0:%c 1:%c 2:%c!!!\n",temp[0],temp[1],temp[2]);
		return -EPERM;
	...
	if(k == 1)
	{
		...
		printk("sym gesture_data.enabled = 1");
	}
	else if(k == 2)
	{
		gesture_data.enabled = 0;
			printk("sym gesture_data.enabled = 0");
	}
	else
		printk("sym switch=%x\n",k);
	printk("sym gtp_gesture_data_write %x,%x,%x,%x,%x\n",temp[0],temp[1],temp[2],temp[3],gesture_data.enabled);
/*
	s32 ret = 0;
	printk("sym gtp_gesture_data_write buff %x,%s\n",*buff,buff);
	ret = copy_from_user(&gesture_data.enabled, buff, 1);
	if (ret) {
		GTP_ERROR("copy_from_user failed.");
		return -EPERM;
	}
	gesture_data.enabled -= 0x30; 
	*/
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	if (DOZE_ENABLED == gesture_data.doze_status) {
		...
		GTP_DEBUG("sym 0x%x = 0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE, doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3]);
     ...
     if (doze_buf[0] == 0xAA)
                {
                    ...
                    u8 type = ((doze_buf[0] & 0x0F) - 0x0A) + (((doze_buf[0] >> 4) & 0x0F) - 0x0A) * 2;
                    
                    GTP_ERROR("sym %s slide to light up the screen!", direction[type]);
		     /*  input_report_key(dev, KEY_RIGHT, 1);
                ...
                else if (doze_buf[0] == 0xBB)
                {
                    ...
                    GTP_INFO("sym %s slide to light up the screen!", direction[type]);
                ...
                {
                    GTP_ERROR("sym Double click to light up the screen!");
...
static long gtp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	u8 *data = NULL;
printk("sym GTP gtp_ioctl cmd %x\n",cmd);
