...
//	node = of_find_matching_node(NULL, touch_of_match);
	if (node) {
		...
		if (ret < 0)
			GTP_ERROR("tpd request_irq IRQ LINE NOT AVAILABLE!.---ret=%d--\n",ret);
		else
			irq_enabled = true;
	} else {
...
client->addr = 0x5d;
	...
	ic_fw_version_g = version_info;
if(tp_sensor_id == 1)
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x","GT915L","TongXingDa",0x5d,ic_fw_version_g,ic_fw_version_g);
else
	len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x","GT915L","DiJin",0x5d,ic_fw_version_g,ic_fw_version_g);
...
#if 1 //ndef CONFIG_GTP_GESTURE_WAKEUP
			gtp_int_sync(25);
#ifdef CONFIG_GTP_ESD_PROTECT
...
static void tpd_suspend(struct device *h)
{
	...
	GTP_INFO("System suspend.");
		GTP_INFO("sym gesture_data.enabled = %x\n",gesture_data.enabled);
#ifdef CONFIG_GTP_PROXIMITY
	if (tpd_proximity_flag == 1)
		return;
#endif
...
{
	printk("sym GT915L series touch panel driver init");
