...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(touch_irq, tpd_eint_interrupt_handler,
			...
			if (ret > 0)
				printk("sym focal tpd request_irq IRQ LINE NOT AVAILABLE! touch_irq:%x.",touch_irq);
	} else {
		printk("sym focal [%s] tpd request_irq can not find touch eint device node!.touch_irq=%x\n", __func__,touch_irq);
	}
	return 0;
}
#if 0
...
/*#ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
	...
	fts_read_reg(client, 0xA8, &vend);
	printk("sym  vend_id= %x \n",vend);
...
pr_err("yfk vend_id = 0x%x\n",vend_id);
	...
	if(vend == 0x82)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,vendorid:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT3327", "helitai", vend,ver,RGK_SAMPLE_HLT_VERSION);
	else if(vend == 0x07)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,vendorid:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT3327", "yikuailai", vend,ver,RGK_SAMPLE_YKL_VERSION);
	else if (vend == 0x67)
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,vendorid:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT3327", "dijing", vend,ver,RGK_SAMPLE_YKL_VERSION);
	else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,vendorid:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT3327", "unknow module", vend,ver,RGK_SAMPLE_YKL_VERSION);
