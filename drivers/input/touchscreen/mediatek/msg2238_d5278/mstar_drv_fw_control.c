...
/*=============================================================*/
#ifdef CONFIG_TP_MSG2238_COMPATIBLE
...
#include "msg22xx_dddd_update_bin.h"
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
		...
		else if (eSwId == MSG22XX_SW_ID_DDDD)
        {
            nUpdateBinMajor = msg22xx_dddd_update_bin[0xBFF5]<<8 | msg22xx_dddd_update_bin[0xBFF4];
            nUpdateBinMinor = msg22xx_dddd_update_bin[0xBFF7]<<8 | msg22xx_dddd_update_bin[0xBFF6];
        }
		...
		if(nMajor == nUpdateBinMajor) //calm add major number compare logic
		{
        if (nUpdateBinMinor > nMinor)
        {
			...
			else if (eSwId == MSG22XX_SW_ID_DDDD)
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_dddd_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_dddd_update_bin[i*1024]), 512);
                    }
                }
            }
            else
		...
		{
			DBG(&g_I2cClient->dev, "[calm]The update bin version nUpdateBinMajor is error!.\n");
		}
    }
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    //daizhiling boardinfo start
    int len;
    unsigned char tp_info[512];
    //daizhiling boardinfo end
    /*[start]calm: fireware version distinguish*/
    unsigned short pMajor=0;
	unsigned short pMinor=0;
	unsigned char *ppVersion=NULL;
	unsigned short ver_flag=0;
	/*[end]calm*/
	...
	if(g_IsUpdateFirmware ==0 )
	{
		...
		if(pMajor==0x01){
			ver_flag=0x01;
			len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x,ver_flag:0x%x","MSG2238","HELITAI",g_I2cClient->addr,pMajor,pMinor,ver_flag);
		}else if(pMajor==0x0c){
			ver_flag=0x01;
			len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x,ver_flag:0x%x","MSG2238","JUNDA",g_I2cClient->addr,pMajor,pMinor,ver_flag);
		}else if(pMajor==0x02){
			ver_flag=0x01;
			len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x,ver_flag:0x%x","MSG2238","DJN",g_I2cClient->addr,pMajor,pMinor,ver_flag);
		}else{
			ver_flag=0x00;
			len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x,ver_flag:0x%x","unknow","unknow",g_I2cClient->addr,pMajor,pMinor,ver_flag);
		}
		/*[end]calm*/
		
	     //daizhiling boardinfo start
	    //len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x","GT9157",module_name_g,client->addr,ic_fw_version_g,RGK_SAMPLE_VERSION);
	    //len=sprintf(tp_info,"TP IC:%s, TP I2C ADR:0x%x","MSG2238",client->addr);
