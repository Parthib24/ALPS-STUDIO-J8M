...
//        tsps_assist_register_callback("msg2xxx", &DrvPlatformLyrTpPsEnable, &DrvPlatformLyrGetTpPsData);
	...
	#ifndef CONFIG_DUMMY_PROXIMITY_SENSOR
	...
	#else
		if(mt_set_gpio_dir(GPIO_DUMMY_PROXIMITY_SENSOR_PIN, GPIO_DIR_IN))
		{
			printk(KERN_ERR "[calm msg2238]mt_set_gpio_dir fail!!\n");
		}
		if(mt_get_gpio_in(GPIO_DUMMY_PROXIMITY_SENSOR_PIN)){ /* project d5175 use GPIO4 */
			/*calm: use real psensor */
			DBG(&g_I2cClient->dev, "Use real psensor! Please check it!\n");
		}else{
			/*calm: use dummy psensor from touchscreen*/
			DBG(&g_I2cClient->dev, "Use dummy psensor...!\n");
	...
	#endif //CONFIG_DUMMY_PROXIMITY_SENSOR
#endif
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
    }
    else
