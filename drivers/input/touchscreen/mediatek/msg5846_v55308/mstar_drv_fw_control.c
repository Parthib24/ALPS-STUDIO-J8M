...
u8 firmware_from_headfile; //rl add
u8 firmware_from_reg;
...
/*=============================================================*/
#if 0//rl add for build sucess when open CONFIG_UPDATE_FIRMWARE_BY_SW_ID
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void) 
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
	 ...
	 firmware_from_headfile = nUpdateBinMinor;//rl add
        DrvFwCtrlGetCustomerFirmwareVersionByDbBus(EMEM_MAIN, &nMajor, &nMinor, &pVersion);
	 firmware_from_reg =nMinor; //rl add
