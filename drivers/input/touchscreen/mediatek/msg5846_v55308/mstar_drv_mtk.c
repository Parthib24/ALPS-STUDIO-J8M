...
extern u8 firmware_from_headfile;// rl add
extern u8 firmware_from_reg;// rl add
/*=============================================================*/
...
#if 1// rl add
	if(firmware_from_reg >= firmware_from_headfile && firmware_from_reg != 0xff){
		ver_flag = 0x1;
	}else{
		ver_flag = 0x0;
	}
    len=sprintf(tp_info,"TP Module:%s,TP IC:%s,TP I2C ADR:0x%x,SW Firmware:%d,Ver_flag:%d","DJN","MSG5846",client->addr,firmware_from_reg,ver_flag);
    fix_tp_proc_info(tp_info,len);
    printk("tttt firmware_from_reg=%d  firmware_from_headfile=%d\n",firmware_from_reg,firmware_from_headfile);
#endif
