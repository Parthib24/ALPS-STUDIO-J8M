...
#define BUTTON_H (100)
#if 1//rl add
int g_TpVirtualKeyDimLocal[MAX_KEY_NUM][4]; //MAX_KEY_NUM == 4
#else
int g_TpVirtualKeyDimLocal[MAX_KEY_NUM][4] = {{BUTTON_W/2*1,TOUCH_SCREEN_FHD_Y_MAX+BUTTON_H/2,BUTTON_W,BUTTON_H},
                                                    {BUTTON_W/2*3,TOUCH_SCREEN_FHD_Y_MAX+BUTTON_H/2,BUTTON_W,BUTTON_H},
                                                    {BUTTON_W/2*5,TOUCH_SCREEN_FHD_Y_MAX+BUTTON_H/2,BUTTON_W,BUTTON_H},
                                                    {BUTTON_W/2*7,TOUCH_SCREEN_FHD_Y_MAX+BUTTON_H/2,BUTTON_W,BUTTON_H}};
#endif
#endif 
...
////get the correct LCM resolutions
#ifdef ONCELL_HD_OUTCELL_FHD_TOUCH_SCREEN
void DrvPlatformLyrGetCorrectTouchSize(void)
{
        u8 szTxData[3] = {0};
        u8 szRxData[4] = {0};
//	 u16 pMajor = 0;
//	 u16 pMinor = 0;
        ...
        //pMinor &= (szRxData[3]<<8) + szRxData[2];
       	if((szRxData[1]&0x3F) != 0)	///oncell---HD
   		{
   		        DBG(&g_I2cClient->dev, "Mstar_Touch_Screen is HD-Oncell\n"); 

			 TOUCH_SCREEN_X_MAX = TOUCH_SCREEN_HD_X_MAX ;
			// TOUCH_SCREEN_X_MAX = TOUCH_SCREEN_HD_Y_MAX ;
			TOUCH_SCREEN_Y_MAX = TOUCH_SCREEN_HD_Y_MAX ;
#if 1 //rl add to init g_TpVirtualKeyDimLocal in other way
		for(j=0;j<4;j++)
			for(i=0;i<4;i++)
			 {
				if(0 == j)
					g_TpVirtualKeyDimLocal[i][j] = BUTTON_W/2*(2*i+1);
				else if(1 == j)
					g_TpVirtualKeyDimLocal[i][j] = TOUCH_SCREEN_Y_MAX+BUTTON_H/2;
				else if(2 == j)
					g_TpVirtualKeyDimLocal[i][j] = BUTTON_W;
				else if(3 == j)
					g_TpVirtualKeyDimLocal[i][j] = BUTTON_H;
			 }
#endif
   		}
       	else ////Outcell --- FHD
   		{
   		        DBG(&g_I2cClient->dev, "Mstar_Touch_Screen is FHD-Outcell\n"); 

			 TOUCH_SCREEN_X_MAX = TOUCH_SCREEN_FHD_X_MAX ;
			// TOUCH_SCREEN_X_MAX = TOUCH_SCREEN_FHD_X_MAX ;   
			TOUCH_SCREEN_Y_MAX = TOUCH_SCREEN_FHD_Y_MAX ;
#if 1 //rl add to init g_TpVirtualKeyDimLocal in other way
		for(j=0;j<4;j++)
			for(i=0;i<4;i++)
			 {
