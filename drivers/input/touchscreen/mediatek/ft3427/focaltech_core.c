...
static const struct i2c_device_id ft5x0x_tpd_id[] = {{"ft3327", 0}, {} };
static const struct of_device_id ft5x0x_dt_match[] = {
	{.compatible = "mediatek,cap_touch"},// rl add
...
unsigned char tp_info[512];
	...
	printk("ttttt:i2c_client_FT->addr=%d\n",i2c_client->addr);
	...
	TPD_DMESG("mtk_tpd: tpd_probe ft3327\n");

	regulator_enable_flag = regulator_is_enabled(tpd->reg);
	if (!regulator_enable_flag)
...
/*#ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
	...
	if(ver >= uc_host_fm_ver && ver != 0xff){
		ver_flag = 0x1;
	}else{
		ver_flag = 0x0;
	}
    len=sprintf(tp_info,"TP Module:%s,TP IC:%s,TP I2C ADR:0x%x ,SW Firmware:%d,Ver_flag:%d","HLT","FT3427",client->addr,ver,ver_flag);
    fix_tp_proc_info(tp_info,len);
#endif 

TPD_DMESG(TPD_DEVICE " ttt fts_read_reg version : %d \n", ver);
	}
#ifdef CONFIG_MTK_SENSOR_HUB_SUPPORT
	int ret;
...
static void tpd_suspend(struct device *h)
{
	...
	//retval = regulator_disable(tpd->reg); /* fp always on */
	if (retval != 0)
