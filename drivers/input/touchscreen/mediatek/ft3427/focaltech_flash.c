...
static unsigned char CTPM_FW[] = {
	//#include "FT3x27_LD_WLD_L521A(T50PRS75B2F)_V01_D07_20160328_app.i"
	//#include "HLT_B55307_FT3427_V02_D01_20170111_app.i"
	//#include "BY-U55KH28P2M_FT3427_V04_D01_20170216_app.i"
	  #include "BY-U55KH28P2M_FT3427_V06_D01_20170817_app.i"
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("tttt fts_ctpm_auto_upgrade ,new fw id=%d\n",uc_host_fm_ver);
	if (uc_tp_fm_ver == FTS_REG_FW_VER ||   uc_tp_fm_ver < uc_host_fm_ver || 1)
	{
		msleep(100);
		...
		fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver); 
		printk("ttt ver_update=%d\n",uc_tp_fm_ver);
	}

	#if 1// rl add 

	if(uc_tp_fm_ver >= uc_host_fm_ver && uc_tp_fm_ver != 0xff){
		ver_flag = 0x1;
	}else{
		ver_flag = 0x0;
	}
	
    len=sprintf(tp_info,"TP IC:%s,TP module:%s,TP I2C adr:0x%x ,SW FirmWare:%d,Sample FirmWare:%d","FT3427","HLT",client->addr,uc_tp_fm_ver,RGK_FOCALTECH_FW_VERSION);
