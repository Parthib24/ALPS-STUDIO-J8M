...
extern unsigned char g_FwData[MAX_UPDATE_FIRMWARE_BUFFER_SIZE][1024];
...
extern unsigned char g_IsSwitchModeByAPK;

extern unsigned char IS_FIRMWARE_DATA_LOG_ENABLED;
extern unsigned char IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED;

#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
extern unsigned short g_FwPacketDataAddress;
extern unsigned short g_FwPacketFlagAddress;

extern unsigned char g_FwSupportSegment;
...
extern unsigned int g_IsEnableReportRate;
extern unsigned int g_InterruptCount;
extern unsigned int g_ValidTouchCount;

extern struct timeval g_StartTime;
#endif //CONFIG_ENABLE_COUNT_REPORT_RATE

#ifdef CONFIG_ENABLE_HOTKNOT
extern unsigned char g_HotKnotState;
...
//static unsigned char _gTpVendorCode[3] = {0}; // used for MSG28XX

static unsigned char _gOneDimenFwData[MSG28XX_FIRMWARE_WHOLE_SIZE*1024] = {0}; // used for MSG28XX

static unsigned char _gFwDataBuf[MSG28XX_FIRMWARE_WHOLE_SIZE*1024] = {0}; // used for update firmware from SD card
...
static unsigned int _gUpdateRetryCount = UPDATE_FIRMWARE_RETRY_COUNT;
static unsigned int _gIsUpdateInfoBlockFirst = 0;
static struct work_struct _gUpdateFirmwareBySwIdWork;
static struct workqueue_struct *_gUpdateFirmwareBySwIdWorkQueue = NULL;
static unsigned char _gTempData[1024]; 
#endif //CONFIG_UPDATE_FIRMWARE_BY_SW_ID

#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
static unsigned int _gGestureWakeupValue[2] = {0};
#endif //CONFIG_ENABLE_GESTURE_WAKEUP

#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
static unsigned char _gTouchPacketFlag[2] = {0};
#endif //CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA

#ifdef CONFIG_ENABLE_CHARGER_DETECTION
static unsigned char _gChargerPlugIn = 0;
#endif //CONFIG_ENABLE_CHARGER_DETECTION

#ifdef CONFIG_ENABLE_TYPE_B_PROTOCOL
static unsigned char _gPreviousTouch[MAX_TOUCH_NUM] = {0};
static unsigned char _gCurrentTouch[MAX_TOUCH_NUM] = {0};
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL

static unsigned char _gIsDisableFinagerTouch = 0;

static unsigned short _gSFR_ADDR3_BYTE0_1_VALUE = 0x0000;
static unsigned short _gSFR_ADDR3_BYTE2_3_VALUE = 0x0000;
...
unsigned char g_ChipType = 0;
unsigned char g_DemoModePacket[DEMO_MODE_PACKET_LENGTH] = {0};

#ifdef CONFIG_ENABLE_HOTKNOT
unsigned char g_DemoModeHotKnotSndRetPacket[DEMO_HOTKNOT_SEND_RET_LEN] = {0};
unsigned char g_DebugModeHotKnotSndRetPacket[DEBUG_HOTKNOT_SEND_RET_LEN] = {0};
#endif //CONFIG_ENABLE_HOTKNOT

FirmwareInfo_t g_FirmwareInfo;
unsigned char g_LogModePacket[DEBUG_MODE_PACKET_LENGTH] = {0};
unsigned short g_FirmwareMode;

#ifdef CONFIG_ENABLE_GESTURE_WAKEUP

#if defined(CONFIG_ENABLE_GESTURE_DEBUG_MODE)
unsigned char _gGestureWakeupPacket[GESTURE_DEBUG_MODE_PACKET_LENGTH] = {0};
#elif defined(CONFIG_ENABLE_GESTURE_INFORMATION_MODE)
unsigned char _gGestureWakeupPacket[GESTURE_WAKEUP_INFORMATION_PACKET_LENGTH] = {0};
#else
unsigned char _gGestureWakeupPacket[GESTURE_WAKEUP_PACKET_LENGTH] = {0};
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE

#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
unsigned char g_GestureDebugFlag = 0x00;
unsigned char g_GestureDebugMode = 0x00;
unsigned char g_LogGestureDebug[GESTURE_DEBUG_MODE_PACKET_LENGTH] = {0};
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE

#ifdef CONFIG_ENABLE_GESTURE_INFORMATION_MODE
unsigned int g_LogGestureInfor[GESTURE_WAKEUP_INFORMATION_PACKET_LENGTH] = {0};
...
unsigned int g_GestureWakeupMode[2] = {0xFFFFFFFF, 0xFFFFFFFF};
#else                                              // support at most 16 types of gesture wakeup mode
unsigned int g_GestureWakeupMode[2] = {0x0000FFFF, 0x00000000};
#endif //CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE

unsigned char g_GestureWakeupFlag = 0;
#endif //CONFIG_ENABLE_GESTURE_WAKEUP

#ifdef CONFIG_ENABLE_PROXIMITY_DETECTION
unsigned char g_EnableTpProximity = 0;
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
unsigned char g_FaceClosingTp = 0; // for QCOM platform -> 1 : close to, 0 : far away 
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
unsigned char g_FaceClosingTp = 1; // for MTK platform -> 0 : close to, 1 : far away 
#endif
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION

#ifdef CONFIG_ENABLE_CHARGER_DETECTION
unsigned char g_ForceUpdate = 0;
#endif //CONFIG_ENABLE_CHARGER_DETECTION

unsigned char g_IsUpdateFirmware = 0x00;
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
#ifdef CONFIG_ENABLE_GESTURE_INFORMATION_MODE
static void _DrvFwCtrlCoordinate(unsigned char *pRawData, unsigned int *pTranX, unsigned int *pTranY);
#endif //CONFIG_ENABLE_GESTURE_INFORMATION_MODE
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
...
static signed int _DrvFwCtrlParsePacket(unsigned char *pPacket, unsigned short nLength, TouchInfo_t *pInfo)
{
    unsigned int i;
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        else if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x80 && pPacket[3] == PACKET_TYPE_GESTURE_INFORMATION)
        {
            ...
            {
                unsigned int nTranX = 0;
                unsigned int nTranY = 0;
...
//                    input_report_key(g_InputDevice, RESERVER51, 0);
    ...
    if (pPacket[0] == 0x5A)
    {
        unsigned char nButton = pPacket[nLength-2]; //Since the key value is stored in 0th~3th bit of variable "button", we can only retrieve 0th~3th bit of it. 
...
static void _DrvFwCtrlStoreFirmwareData(unsigned char *pBuf, unsigned int nSize)
{
    unsigned int nCount = nSize / 1024;
    unsigned int nRemainder = nSize % 1024;
    unsigned int i;
...
static unsigned short _DrvFwCtrlMsg26xxmGetSwId(EmemType_e eEmemType)
{
    unsigned short nRetVal = 0; 
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
...
static signed int _DrvFwCtrlMsg26xxmUpdateFirmware(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    unsigned int i, j;
    unsigned int nCrcMain, nCrcMainTp;
    unsigned int nCrcInfo, nCrcInfoTp;
    unsigned short nRegData = 0;
...
static void _DrvFwCtrlMsg28xxConvertFwDataTwoDimenToOneDimen(unsigned char szTwoDimenFwData[][1024], unsigned char* pOneDimenFwData)
{
    unsigned int i, j;
...
static unsigned int _DrvFwCtrlMsg28xxCalculateCrc(unsigned char *pFwData, unsigned int nOffset, unsigned int nSize)
{
    unsigned int i;
    unsigned int nData = 0, nCrc = 0;
    unsigned int nCrcRule = 0x0C470C06; // 0000 1100 0100 0111 0000 1100 0000 0110
...
static void _DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(unsigned short nStartAddr, unsigned char *pFirstData, unsigned int nBlockSize, unsigned short nPageNum, EmemType_e eEmemType)
{
    unsigned short nWriteAddr = nStartAddr/4;
...
static void _DrvFwCtrlMsg28xxIspBurstWriteEFlashDoWrite(unsigned char *pBufferData, unsigned int nLength)
{
    unsigned int i;
    unsigned char  szDbBusTxData[3+MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE] = {0};
...
{
    unsigned char szDbBusTxData[1] = {0};
...
static void _DrvFwCtrlMsg28xxWriteEFlashStart(unsigned short nStartAddr, unsigned char *pFirstData, EmemType_e eEmemType)
{
    unsigned short nWriteAddr = nStartAddr/4;
...
static void _DrvFwCtrlMsg28xxWriteEFlashDoWrite(unsigned short nStartAddr, unsigned char *pBufferData)
{
    unsigned short nWriteAddr = nStartAddr/4;
...
static void _DrvFwCtrlMsg28xxReadEFlashStart(unsigned short nStartAddr, EmemType_e eEmemType)
{
    DBG("*** %s() eEmemType = %d ***\n", __func__, eEmemType);
...
static void _DrvFwCtrlMsg28xxReadEFlashDoRead(unsigned short nReadAddr, unsigned char *pReadData)
{
    unsigned short nRegData1 = 0, nRegData2 = 0;
...
/*
static void _DrvFwCtrlMsg28xxGetTpVendorCode(unsigned char *pTpVendorCode)
{
    DBG("*** %s() ***\n", __func__);

    if (g_ChipType == CHIP_TYPE_MSG28XX)
    {
        unsigned short nReadAddr = 0;
        unsigned char  szTmpData[4] = {0};
...
static void _DrvFwCtrlMsg28xxProgramEmem(EmemType_e eEmemType)
{
    ...
    unsigned int k;
#endif
    unsigned int nPageNum = 0, nLength = 0, nIndex = 0, nWordNum = 0;
    unsigned int nRetryTime = 0;
    unsigned char  nRegData = 0;
    unsigned char  szFirstData[MSG28XX_EMEM_SIZE_BYTES_ONE_WORD] = {0};
    unsigned char  szBufferData[MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE] = {0};
...
static unsigned short _DrvFwCtrlMsg28xxGetSwId(EmemType_e eEmemType) 
{
    unsigned short nRetVal = 0; 
    unsigned short nReadAddr = 0;
    unsigned char  szTmpData[4] = {0};
...
static unsigned int _DrvFwCtrlMsg28xxGetFirmwareCrcByHardware(EmemType_e eEmemType) 
{
    unsigned int nRetVal = 0; 
    unsigned int nRetryTime = 0;
    unsigned int nCrcEndAddr = 0;
...
static unsigned int _DrvFwCtrlMsg28xxRetrieveFirmwareCrcFromEFlash(EmemType_e eEmemType) 
{
    unsigned int nRetVal = 0; 
    unsigned short nReadAddr = 0;
    unsigned char  szTmpData[4] = {0};
...
static unsigned int _DrvFwCtrlMsg28xxRetrieveFrimwareCrcFromBinFile(unsigned char szTmpBuf[][1024], EmemType_e eEmemType) 
{
    unsigned int nRetVal = 0; 
...
static signed int _DrvFwCtrlMsg28xxCheckFirmwareBinIntegrity(unsigned char szFwData[][1024])
{
    unsigned int nCrcMain = 0, nCrcMainBin = 0;
    unsigned int nCrcInfo = 0, nCrcInfoBin = 0;
    unsigned int nRetVal = 0;
...
static signed int _DrvFwCtrlMsg28xxUpdateFirmware(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    unsigned int nCrcMain = 0, nCrcMainHardware = 0, nCrcMainEflash = 0;
    unsigned int nCrcInfo = 0, nCrcInfoHardware = 0, nCrcInfoEflash = 0;
...
static signed int _DrvFwCtrlUpdateFirmwareCash(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    DBG("*** %s() ***\n", __func__);
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX) // (0x85)
    {
        ...
        {
            unsigned short eSwId = 0x0000;
            unsigned short eVendorId = 0x0000;
...
static signed int _DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    signed int nRetVal = 0;
...
void DrvFwCtrlOpenGestureWakeup(unsigned int *pMode)
{
...
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
void DrvFwCtrlOpenGestureDebugMode(unsigned char nGestureFlag)
{
    unsigned char szDbBusTxData[3] = {0};
    signed int rc;
...
unsigned int DrvFwCtrlReadDQMemValue(unsigned short nAddr)
{
...
void DrvFwCtrlWriteDQMemValue(unsigned short nAddr, unsigned int nData)
{
    unsigned char szDbBusTxData[7] = {0};
...
static unsigned int _DrvFwCtrlMsg26xxmCalculateFirmwareCrcFromEFlash(EmemType_e eEmemType, unsigned char nIsNeedResetHW)
{
    unsigned int nRetVal = 0; 
    unsigned short nRegData = 0;
...
static unsigned int _DrvFwCtrlMsg26xxmRetrieveFirmwareCrcFromMainBlock(EmemType_e eEmemType)
{
    unsigned int nRetVal = 0; 
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
...
static unsigned int _DrvFwCtrlMsg26xxmRetrieveInfoCrcFromInfoBlock(void)
{
    unsigned int nRetVal = 0; 
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
...
static unsigned int _DrvFwCtrlMsg26xxmRetrieveFrimwareCrcFromBinFile(unsigned char szTmpBuf[][1024], EmemType_e eEmemType)
{
    unsigned int nRetVal = 0; 
...
static unsigned int _DrvFwCtrlMsg26xxmCalculateInfoCrcByDeviceDriver(void)
{
    unsigned int nRetVal = 0xffffffff; 
    unsigned int i, j;
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
...
static signed int _DrvFwCtrlMsg26xxmCompare8BytesForCrc(unsigned char szTmpBuf[][1024])
{
    signed int nRetVal = -1;
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
    unsigned char szDbBusRxData[8] = {0};
...
static void _DrvFwCtrlMsg26xxmProgramFirmwareOnEFlash(EmemType_e eEmemType)
{
    ...
    unsigned int i, j; 
    unsigned short nRegData = 0;
//    unsigned short nRegData2 = 0, nRegData3 = 0; // add for debug
...
static signed int _DrvFwCtrlMsg26xxmUpdateFirmwareBySwId(void)
{
    signed int nRetVal = -1;
...
{
    unsigned int nCrcMainA, nCrcInfoA, nCrcMainB, nCrcInfoB;
    unsigned int i;
...
/*
static signed int _DrvFwCtrlMsg28xxUpdateFirmwareBySwId(void) 
{
    signed int nRetVal = -1;
    unsigned int nCrcInfoA = 0, nCrcInfoB = 0, nCrcMainA = 0, nCrcMainB = 0;
...
{
    unsigned int nCrcMainA, nCrcMainB;
    unsigned int i;
    unsigned short nUpdateBinMajor = 0, nUpdateBinMinor = 0;
    unsigned short nMajor = 0, nMinor = 0;
    unsigned char *pVersion = NULL;
...
{
    unsigned int i;
    unsigned char szDbBusTxData[35] = {0};
...
unsigned char DrvFwCtrlGetChipType(void)
{
    signed int rc =0;
    unsigned char nChipType = 0;
...
void DrvFwCtrlGetCustomerFirmwareVersionByDbBus(EmemType_e eEmemType, unsigned short *pMajor, unsigned short *pMinor, unsigned char **ppVersion) // support MSG28xx only
{
    unsigned short nReadAddr = 0;
    unsigned char  szTmpData[4] = {0};
...
void DrvFwCtrlGetCustomerFirmwareVersion(unsigned short *pMajor, unsigned short *pMinor, unsigned char **ppVersion)
{
    unsigned char szDbBusTxData[3] = {0};
    unsigned char szDbBusRxData[4] = {0};
...
void DrvFwCtrlGetPlatformFirmwareVersion(unsigned char **ppVersion)
{
    DBG("*** %s() ***\n", __func__);
    ...
    if (g_ChipType == CHIP_TYPE_MSG26XXM)   
    {
        ...
        unsigned short nRegData = 0;
        unsigned char szDbBusTxData[5] = {0};
        unsigned char szDbBusRxData[16] = {0};
        ...
        if (*ppVersion == NULL)
        {
            *ppVersion = kzalloc(sizeof(unsigned char)*16, GFP_KERNEL);
        }
    ...
    {
        unsigned char szDbBusTxData[1] = {0};
        unsigned char szDbBusRxData[10] = {0};
        ...
        if (*ppVersion == NULL)
        {
            *ppVersion = kzalloc(sizeof(unsigned char)*10, GFP_KERNEL);
        }
...
signed int DrvFwCtrlUpdateFirmware(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    DBG("*** %s() ***\n", __func__);
...
signed int DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    signed int nRetVal = -1;
...
unsigned short DrvFwCtrlGetFirmwareMode(void) // use for MSG26XXM only
{
...
unsigned short DrvFwCtrlChangeFirmwareMode(unsigned short nMode)
{
    DBG("*** %s() *** nMode = 0x%x\n", __func__, nMode);
    ...
    {
        unsigned char szDbBusTxData[2] = {0};
        unsigned int i = 0;
        signed int rc;
        ...
        szDbBusTxData[1] = (unsigned char)nMode;
...
#ifdef CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA
void DrvFwCtrlGetTouchPacketAddress(unsigned short* pDataAddress, unsigned short* pFlagAddress)
{
    signed int rc = 0;
    unsigned int i = 0;
    unsigned char szDbBusTxData[1] = {0};
...
signed int DrvFwCtrlEnableProximity(void)
{
    unsigned char szDbBusTxData[4] = {0};
    signed int rc;
...
signed int DrvFwCtrlDisableProximity(void)
{
    unsigned char szDbBusTxData[4] = {0};
    signed int rc;
...
void DrvFwCtrlGetGloveInfo(unsigned char *pGloveMode)
{
    unsigned char szDbBusTxData[3] = {0};
    unsigned char szDbBusRxData[2] = {0};
    unsigned int i = 0;
    signed int rc;
...
void DrvFwCtrlChargerDetection(unsigned char nChargerStatus)
{
    unsigned int i = 0;
    unsigned char szDbBusTxData[2] = {0};
    signed int rc = 0;
...
static signed int _DrvFwCtrlReadFingerTouchData(unsigned char *pPacket, unsigned short nReportPacketLength)
{
    signed int rc;
...
void DrvFwCtrlHandleFingerTouch(void)
{
    TouchInfo_t tInfo;
    unsigned int i = 0;
    static unsigned int nLastKeyCode = 0xFF;
    static unsigned int nLastCount = 0;
    unsigned char *pPacket = NULL;
    unsigned short nReportPacketLength = 0;
