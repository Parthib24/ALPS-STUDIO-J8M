...
static int MS_TS_MSG_IC_GPIO_RST = 60; // Must set a value other than 1
static int MS_TS_MSG_IC_GPIO_INT = 10; // Must set value as 1
#endif //CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
#endif
...
{
    DBG(&g_I2cClient->dev, "*** %s() *** MS_TS_MSG_IC_GPIO_RST=%x\n", __func__,MS_TS_MSG_IC_GPIO_RST); 
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
printk("sym DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler MS_TS_MSG_IC_GPIO_INT=%x\n",MS_TS_MSG_IC_GPIO_INT);
...
//                      "msg2xxx", NULL); 
		...
		printk("sym %s MS_TS_MSG_IC_GPIO_INT=%x\n",__func__,MS_TS_MSG_IC_GPIO_INT);
