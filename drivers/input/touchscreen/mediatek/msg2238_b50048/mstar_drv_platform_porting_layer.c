...
extern unsigned char g_FaceClosingTp;
extern unsigned char g_EnableTpProximity;
...
extern unsigned int g_IsInMpTest;
...
#ifdef CONFIG_ENABLE_TOUCH_PIN_CONTROL
static signed int _DrvPlatformLyrTouchPinCtrlInit(struct i2c_client *pClient)
{
    signed int nRetVal = 0;
...
static irqreturn_t _DrvPlatformLyrFingerTouchInterruptHandler(signed int nIrq, void *pDeviceId)
{
    unsigned long nIrqFlag;
...
*/
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
//static irqreturn_t _DrvPlatformLyrFingerTouchInterruptHandler(signed int nIrq, struct irq_desc *desc)
static irqreturn_t _DrvPlatformLyrFingerTouchInterruptHandler(signed int nIrq, void *pDeviceId)
#else
...
void DrvPlatformLyrFingerTouchPressed(signed int nX, signed int nY, signed int nPressure, signed int nId)
{
    DBG("*** %s() ***\n", __func__);
...
void DrvPlatformLyrFingerTouchReleased(signed int nX, signed int nY, signed int nId)
{
    DBG("*** %s() ***\n", __func__);
...
signed int DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
    signed int nRetVal = 0;
...
signed int DrvPlatformLyrTouchDeviceRequestGPIO(struct i2c_client *pClient)
{
    signed int nRetVal = 0;
...
signed int DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    signed int nRetVal = 0;
...
signed int DrvPlatformLyrTouchDeviceRemove(struct i2c_client *pClient)
{
    DBG("*** %s() ***\n", __func__);
...
void DrvPlatformLyrSetIicDataRate(struct i2c_client *pClient, unsigned int nIicDataRate)
{
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
int DrvPlatformLyrTpPsOperate(void* pSelf, unsigned int nCommand, void* pBuffIn, int nSizeIn,
