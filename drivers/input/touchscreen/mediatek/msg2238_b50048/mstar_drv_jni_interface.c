...
static unsigned char * _gSndCmdData = NULL;
static unsigned char * _gRtnCmdData = NULL;



void _DebugJniShowArray(unsigned char *pBuf, unsigned short nLen)
{
    int i;
...
{
    long nRet = 0;
    unsigned char nBusType = 0;
    unsigned short nReadLen = 0;
    unsigned char szCmdData[20] = {0};
...
void CreateMsgToolMem(void)
{
	...
	_gSndCmdData = (unsigned char*)kmalloc(1024, GFP_KERNEL );	
	_gRtnCmdData = (unsigned char*)kmalloc(1024, GFP_KERNEL );           
}
