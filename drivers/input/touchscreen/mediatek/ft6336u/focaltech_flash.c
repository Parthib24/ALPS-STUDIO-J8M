...
static unsigned char CTPM_DJ_FW[] = {
	#include "B50040_480x854_FT6336U_Ver0x11_20170503_app.i"
};
...
int fts_6336GU_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	u8 auc_i2c_write_buf[10];
	u8 bt_ecc;
printk("sym fts_6336GU_ctpm_fw_upgrade\n");
	if(pbt_buf[0] != 0x02)
	{
		TPD_DEBUG("[FTS] FW first byte is not 0x02. so it is invalid \n");
		return -1;
	...
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	msleep(300);	
printk("sym fts_6336GU_ctpm_fw_upgrade end\n");
	return 0;
}
/************************************************************************
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	{
		printk("yfk is_5336_fwsize_30 = 1");
		is_5336_fwsize_30 = 1;
	}
	else 
	{
		printk("yfk is_5336_fwsize_30 = 0");
...
printk("sym reg_val[0] =%x,reg_val[1] =%x,fts_updateinfo_curr.upgrade_id_1 =%x,fts_updateinfo_curr.upgrade_id_2=%x\n",reg_val[0] ,reg_val[1],fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
	...
	if (reg_val[0] <= 4)
	{
		printk("is_5336_new_bootloader BL_VERSION_LZ4 ");
		is_5336_new_bootloader = BL_VERSION_LZ4 ;
	}
	else if(reg_val[0] == 7)
	{printk("is_5336_new_bootloader BL_VERSION_Z7 ");
		is_5336_new_bootloader = BL_VERSION_Z7 ;
	}
	else if(reg_val[0] >= 0x0f)
	{printk("is_5336_new_bootloader BL_VERSION_GZF ");
...
//sym modified for machine restart when tp detects bad in 20150901
	static struct i2c_client *fts_client = NULL;
	u8 fts_ctpm_update(struct i2c_client *client)
	{

		struct task_struct *thread = NULL;

		fts_client = client;
		printk("sym fts_ctpm_update Ready to run update thread");

		thread = kthread_run(fts_ctpm_auto_upgrade, (void *)NULL, "fts_update");

		if (IS_ERR(thread)) {
			printk("fts Failed to create update thread.\n");
			return -1;
		}
...
int fts_ctpm_auto_upgrade(void *data)
{
	u8 uc_host_fm_ver = FTS_REG_FW_VER;
	static u8 uc_tp_fm_ver = 0;
	u8 count = 0;
	int i_ret;
	struct i2c_client *client = fts_client;
...
fts_updateinfo_curr.CHIP_ID = 0x64;
	if ((uc_tp_fm_ver < uc_host_fm_ver )  &&   CTPM_FW!=NULL    || uc_tp_fm_ver <= 0 )
//if (sw_fw_len !=0 && (uc_tp_fm_ver < uc_host_fm_ver ||   uc_tp_fm_ver ==0  ))
	{
		msleep(100);
