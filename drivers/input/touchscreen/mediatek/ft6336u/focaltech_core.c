...
static u8 tpd_proximity_detect 		= 10;//0-->close ; 1--> far away
...
static int read_vend_id(struct i2c_client *client)
{
	int num = 0;
	int ret = 0;
	ret = fts_read_reg(client,FTS_REG_VENDOR_ID,&vend_id);
	while(ret != 1 && num < 5)
	{
...
static struct class *ft6336_class;

static ssize_t enable_ft6336_show(struct class *dev, struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s:%d\n", "gesenable",TP_gesture_Switch);
}

static ssize_t enable_ft6336_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
	unsigned long state;
	ssize_t ret = -EINVAL;
...
static CLASS_ATTR(gesenable, 00774, enable_ft6336_show, enable_ft6336_store);
...
static int tpd_enable_ps(int enable)
{
	...
	//printk("sym tpd_enable_ps [proxi_fts]read: 999 0xb0's value is 0x%02X\n", state);
		printk("sym tpd tpd_enable_ps enable=%x,tpd_proximity_flag=%x,tpd_proximity_flag_one=%x\n",enable,tpd_proximity_flag,tpd_proximity_flag_one);
	...
	if (enable){
		...
		printk("sym tpd tpd_enable_ps enable=%x,tpd_proximity_flag=%x\n",enable,tpd_proximity_flag);
...
static int tpd_enable_ps_ext(int enable)
{
	u8 state;
	int ret = -1;

	i2c_smbus_read_i2c_block_data(i2c_client, 0xB0, 1, &state);
	printk("sym tpd_enable_ps_ext [proxi_fts]read: 999 0xb0's value is 0x%02X\n", state);
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			{				
				value = *(int *)buff_in;
					printk("sym ft6336 tpd_ps_operate SENSOR_ENABLE value=%x\n",value);
				if(value)
				{		
		...
		if(TP_gesture_Switch){
			...
			//printk("tpd fts_read_Gestruedata state=%d\n",state);
			 ...
			 if (tpd_proximity_flag == 1)
			 {
				...
				i2c_smbus_read_i2c_block_data(i2c_client, 0xB0, 1, &state);
	            		printk("sym touch_event_handler tpd_enable_ps_ext state value is 1131 0x%02X\n", state);
				if(!(state&0x01))
				{
					tpd_enable_ps_ext(1);
				}
				...
				{
					tpd_proximity_detect = 10;
				}
	printk("sym 2  tpd_proximity_flag=%x  tpd_proximity_detect=%x\n",tpd_proximity_flag,tpd_proximity_detect);
				TPD_PROXIMITY_DEBUG("sym tpd_proximity_detect 1149 = %d\n", tpd_proximity_detect);
				if ((err = tpd_read_ps()))
				{
				...
				sensor_data.value_divide = 1;
				sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
			//	printk("sym ps value =%x\n",sensor_data.values[0]);
				if ((err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data)))
				{
					TPD_PROXIMITY_DMESG("sym proxi_5206 call hwmsen_get_interrupt_data failed= %d\n", err);	
				}
			}  
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (retval != 0)
		TPD_DMESG("sym Failed to disable reg-vgp6: %d\n", retval);
...
*/
		...
		ft6336_class = class_create(THIS_MODULE, "syna");
	err = class_create_file(ft6336_class, &class_attr_gesenable);
	if (err) {
		printk("ft6336 gensenable create sys interface ERROR\n");
	}
#endif
	...
	fts_get_upgrade_array();
		...
		is_update = true;
	fts_ctpm_update(i2c_client);  //fts_ctpm_auto_upgrade(i2c_client);
		is_update = false;
	#endif
...
*/
	int len;
unsigned char tp_info[512];
pr_err("yfk vend_id = 0x%x\n",vend_id);
	read_vend_id(i2c_client);
	//if (vend_id ==0x67)	
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT6336U", "helitai", client->addr,ver,RGK_SAMPLE_HELITAI_VERSION);
	...
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", "unknow modules", client->addr,ver,0);	
	pr_err("yfk SW FirmWare = 0x%x\n",ver);
	pr_err("yfk tp_info = %s\n",tp_info);*/
...
static void tpd_resume(struct device *h)
{
	...
	printk("sym tpd_resume wake up tpd_proximity_flag=%x,tpd_proximity_flag_one=%x,TP_gesture_Switch=%x\n",tpd_proximity_flag,tpd_proximity_flag_one,TP_gesture_Switch);
	...
	if (retval != 0)
		...
		if (tpd_proximity_flag == 1)
		{
			if(tpd_proximity_flag_one == 1)    //sym note for when tp is suspend,then call comes
			{
				tpd_proximity_flag_one = 0;	
				TPD_DMESG(TPD_DEVICE " tpd_proximity_flag_one \n"); 
				printk("sym hh tpd_resume return;\n");
				return;
			}
		}
	#endif	
...
static void tpd_suspend(struct device *h)
{
	...
	u8 state = 0;
	#endif
	printk("sym tpd_suspend  tpd_proximity_flag=%x,tpd_proximity_flag_one=%x,TP_gesture_Switch=%x\n",tpd_proximity_flag,tpd_proximity_flag_one,TP_gesture_Switch);
	
	#ifdef TPD_PROXIMITY
	if (tpd_proximity_flag == 1)
	{
		tpd_proximity_flag_one = 1;	
			printk("sym hh tpd_suspend return;\n");
		return;
	}
	#endif
...
printk("sym ft6336 suspend regulator_disable \n");
	retval = regulator_disable(tpd->reg);
	if (retval != 0)
		TPD_DMESG("sym Failed to disable reg-vgp6: %d\n", retval);
