...
static unsigned char _gTpVendorCode[3] = {0};
static unsigned char _gOneDimenFwData[MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE*1024+MSG22XX_FIRMWARE_INFO_BLOCK_SIZE] = {0}; // used for MSG22XX
 ...
 */
#include "msg22xx_hlt_v112_update_bin.h"
#include "MSG2238_5001X_dijin2_v3.2.h"
#include "msg22xx_yyyy_update_v2.3.h"
...
#else
static unsigned char _gDwIicInfoData[1024];
...
static unsigned char _gCurrPress[MAX_TOUCH_NUM] = {0};
...
#else
unsigned char _gGestureWakeupPacket[DEMO_MODE_PACKET_LENGTH] = {0}; // for MSG21XXA : packet length(DEMO_MODE_PACKET_LENGTH) , for MSG22XX : packet length(GESTURE_WAKEUP_PACKET_LENGTH)
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE
...
unsigned char g_FaceClosingTp = 10; // for MTK platform -> 0 : close to, 1 : far away 
#endif
#endif //CONFIG_ENABLE_PROXIMITY_DETECTION
...
static signed int _DrvFwCtrlMsg22xxUpdateFirmware(unsigned char szFwData[][1024], EmemType_e eEmemType);
...
static void _DrvFwCtrlMsg22xxGetTpVendorCode(unsigned char *pTpVendorCode)
{
    DBG("*** %s() ***\n", __func__);
...
static unsigned short _DrvFwCtrlMsg22xxGetTrimByte1(void)
{
...
static void _DrvFwCtrlMsg22xxProgramEmem(EmemType_e eEmemType) 
{
    ...
    unsigned char szDbBusTxData[1024] = {0};
#ifdef CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K
    unsigned int nSizePerWrite = 1;
#else
    unsigned int nSizePerWrite = 1021;
#endif //CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K
#endif
...
static unsigned int _DrvFwCtrlMsg22xxGetFirmwareCrcByHardware(EmemType_e eEmemType) 
{
...
static void _DrvFwCtrlMsg22xxConvertFwDataTwoDimenToOneDimen(unsigned char szTwoDimenFwData[][1024], unsigned char* pOneDimenFwData)
{
    unsigned int i, j;
...
signed int DrvFwCtrlEnableProximity_exp(void);
...
static signed int _DrvFwCtrlParsePacket(unsigned char *pPacket, unsigned short nLength, TouchInfo_t *pInfo)
{
    ...
    unsigned int nDeltaX = 0, nDeltaY = 0;
    unsigned int nX = 0;
    unsigned int nY = 0;
    ...
    static unsigned short szPrevX[MAX_TOUCH_NUM] = {0xFFFF, 0xFFFF};
    static unsigned short szPrevY[MAX_TOUCH_NUM] = {0xFFFF, 0xFFFF};
    static unsigned char  szPrevPress[MAX_TOUCH_NUM] = {0};
    unsigned int i = 0;
    unsigned short szX[MAX_TOUCH_NUM] = {0};
    unsigned short szY[MAX_TOUCH_NUM] = {0};
    unsigned short nTemp = 0;
    unsigned char  nChangePoints = 0;
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL
    unsigned char nCheckSumIndex = nLength-1; //Set default checksum index for demo mode
...
//            if ((nDeltaX == 0) && (nDeltaY == 0))
            ...
            {   /* two touch points */
                ...
                nX2 = (unsigned int)(nX + nDeltaX);
                nY2 = (unsigned int)(nY + nDeltaY);
...
static unsigned short _DrvFwCtrlMsg21xxaGetSwId(EmemType_e eEmemType) 
{
...
static unsigned short _DrvFwCtrlMsg22xxGetSwId(EmemType_e eEmemType) 
{
    unsigned short nRetVal = 0; 
...
static unsigned int _DrvFwCtrlMsg22xxRetrieveFirmwareCrcFromEFlash(EmemType_e eEmemType) 
{
    unsigned int nRetVal = 0; 
...
static unsigned int _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(unsigned char szTmpBuf[], EmemType_e eEmemType) 
{
    unsigned int nRetVal = 0; 
...
static signed int _DrvFwCtrlMsg22xxUpdateFirmwareBySwId(void) 
{
...
static unsigned int _DrvFwCtrlMsg21xxaCalculateMainCrcFromEFlash(void) 
{
    unsigned int nRetVal = 0; 
    unsigned short nRegData = 0;
...
static unsigned int _DrvFwCtrlMsg21xxaRetrieveMainCrcFromMainBlock(void) 
{
    unsigned int nRetVal = 0; 
    unsigned short nRegData = 0;
    unsigned char szDbBusTxData[5] = {0};
...
static signed int _DrvFwCtrlMsg21xxaUpdateFirmwareBySwId(unsigned char szFwData[][1024], EmemType_e eEmemType) 
{
    unsigned int i, j, nCalculateCrcSize;
    unsigned int nCrcMain = 0, nCrcMainTp = 0;
    unsigned int nCrcInfo = 0, nCrcInfoTp = 0;
    unsigned int nCrcTemp = 0;
    unsigned short nRegData = 0;
...
void _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId(void) 
{
    ...
    unsigned short nUpdateBinMajor = 0, nUpdateBinMinor = 0;
    unsigned short nMajor = 0, nMinor = 0;
    unsigned char nIsCompareVersion = 0;
    unsigned char *pVersion = NULL; 
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
	...
	printk("sym _gTpVendorCode0=%x,_gTpVendorCode1=%x,_gTpVendorCode2=%x",_gTpVendorCode[0],_gTpVendorCode[1],_gTpVendorCode[2]);
...
//	extern int  fix_tp_proc_info(void  *tp_data, unsigned char data_len);
	...
	if(pMajor==0x2){
		len=sprintf(tp_info,"UTP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","DIJING",0x26,pMajor,pMinor,SAMPLE_FWVER_DJ1_MAJOR,SAMPLE_FWVER_DJ1_MINOR);
	}
	else if(pMajor==0x1){
		...
		len=sprintf(tp_info,"UTP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","DIJING",0x26,pMajor,pMinor,SAMPLE_FWVER_DJ2_MAJOR,SAMPLE_FWVER_DJ2_MINOR);
	}
	else{
		len=sprintf(tp_info,"UTP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","UNKOWN","UNKOWN",NULL,0,0,0,0);	
	}
...
static signed int _DrvFwCtrlUpdateFirmwareC32(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    unsigned int i, j;
    unsigned int nCrcMain, nCrcMainTp;
    unsigned int nCrcInfo, nCrcInfoTp;
    unsigned int nCrcTemp;
    unsigned short nRegData = 0;
...
static signed int _DrvFwCtrlUpdateFirmwareC33(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    unsigned char szLifeCounter[2];
    unsigned int i, j;
    unsigned int nCrcMain, nCrcMainTp;
    unsigned int nCrcInfo, nCrcInfoTp;
    unsigned int nCrcTemp;
...
static signed int _DrvFwCtrlMsg22xxUpdateFirmware(unsigned char szFwData[][1024], EmemType_e eEmemType)
{
    ...
    unsigned int nCrcMain = 0, nCrcMainTp = 0;
    unsigned int nCrcInfo = 0, nCrcInfoTp = 0;
    unsigned int nRemainSize, nBlockSize, nSize;
    unsigned int nTimeOut = 0;
    unsigned short nRegData = 0;
    unsigned short nTrimByte1 = 0;
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
...
//	extern int  fix_tp_proc_info(void  *tp_data, unsigned char data_len);
	...
	if(pMajor==0xc){
		len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","DIJING",0x26,pMajor,pMinor,12,1);
	}else{
len=sprintf(tp_info,"TP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","HELITAI",0x26,pMajor,pMinor,1,4);
	}
...
static signed int _DrvFwCtrlUpdateFirmwareCash(unsigned char szFwData[][1024])
{
    DBG("*** %s() ***\n", __func__);
    ...
    {
//        unsigned short nChipType;
        unsigned char nChipVersion = 0;
...
{
    unsigned int i;
    unsigned char szDbBusTxData[27] = {0};
...
void DrvFwCtrlGetPlatformFirmwareVersion(unsigned char **ppVersion)
{
    unsigned int i;
    unsigned short nRegData1, nRegData2;
    unsigned char szDbBusRxData[12] = {0};
...
void DrvFwCtrlHandleFingerTouch(void)
{
    TouchInfo_t tInfo;
    unsigned int i;
    unsigned char nTouchKeyCode = 0;
    static unsigned int nLastKeyCode = 0;
