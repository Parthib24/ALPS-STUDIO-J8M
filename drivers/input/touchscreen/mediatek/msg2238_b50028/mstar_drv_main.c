...
extern unsigned char g_LogModePacket[DEBUG_MODE_PACKET_LENGTH];
extern unsigned short g_FirmwareMode;

#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
extern unsigned int g_GestureWakeupMode[2];

#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
extern unsigned char g_LogGestureDebug[128];
extern unsigned char g_GestureDebugFlag;
extern unsigned char g_GestureDebugMode;

extern struct input_dev *g_InputDevice;
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE

#ifdef CONFIG_ENABLE_GESTURE_INFORMATION_MODE
extern unsigned int g_LogGestureInfor[GESTURE_WAKEUP_INFORMATION_PACKET_LENGTH];
...
static unsigned short _gDebugReg[MAX_DEBUG_REGISTER_NUM] = {0};
static unsigned short _gDebugRegValue[MAX_DEBUG_REGISTER_NUM] = {0};
static unsigned int _gDebugRegCount = 0;

static unsigned char _gDebugCmdArgu[MAX_DEBUG_COMMAND_ARGUMENT_NUM] = {0};
static unsigned short _gDebugCmdArguCount = 0;
static unsigned int _gDebugReadDataSize = 0;

static char _gDebugBuf[1024] = {0};

static unsigned char *_gPlatformFwVersion = NULL; // internal use firmware version for MStar
...
#ifdef CONFIG_ENABLE_GESTURE_INFORMATION_MODE
static unsigned int _gLogGestureCount = 0;
static unsigned char _gLogGestureInforType = 0;
#endif //CONFIG_ENABLE_GESTURE_INFORMATION_MODE

#endif //CONFIG_ENABLE_GESTURE_WAKEUP

static unsigned int _gIsUpdateComplete = 0;

static unsigned char *_gFwVersion = NULL; // customer firmware version

static unsigned int _gFeatureSupportStatus = 0;
...
unsigned int SLAVE_I2C_ID_DBBUS = (0xC4>>1); //0x62 // for MSG21XX/MSG21XXA/MSG26XXM/MSG28XX
//unsigned int SLAVE_I2C_ID_DBBUS = (0xB2>>1); //0x59 // for MSG22XX
unsigned int SLAVE_I2C_ID_DWI2C = (0x4C>>1); //0x26 


unsigned short FIRMWARE_MODE_UNKNOWN_MODE = 0xFFFF;
unsigned short FIRMWARE_MODE_DEMO_MODE = 0xFFFF;
unsigned short FIRMWARE_MODE_DEBUG_MODE = 0xFFFF;
unsigned short FIRMWARE_MODE_RAW_DATA_MODE = 0xFFFF;

struct kset *g_TouchKSet = NULL;
struct kobject *g_TouchKObj = NULL;
unsigned char g_IsSwitchModeByAPK = 0;


unsigned char IS_GESTURE_WAKEUP_ENABLED = 0;
unsigned char IS_GESTURE_DEBUG_MODE_ENABLED = 0;
unsigned char IS_GESTURE_INFORMATION_MODE_ENABLED = 0;
unsigned char IS_GESTURE_WAKEUP_MODE_SUPPORT_64_TYPES_ENABLED = 0;

unsigned char TOUCH_DRIVER_DEBUG_LOG_LEVEL = CONFIG_TOUCH_DRIVER_DEBUG_LOG_LEVEL;
unsigned char IS_FIRMWARE_DATA_LOG_ENABLED = CONFIG_ENABLE_FIRMWARE_DATA_LOG;
unsigned char IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED = 0;
...
unsigned short g_FwPacketDataAddress = 0;
unsigned short g_FwPacketFlagAddress = 0;
unsigned char g_FwSupportSegment = 0;
#endif //CONFIG_ENABLE_SEGMENT_READ_FINGER_TOUCH_DATA

#ifdef CONFIG_ENABLE_COUNT_REPORT_RATE
unsigned int g_IsEnableReportRate = 0;
unsigned int g_InterruptCount = 0;
unsigned int g_ValidTouchCount = 0;
unsigned int g_InterruptReportRate = 0;
unsigned int g_ValidTouchReportRate = 0;

struct timeval g_StartTime;
#endif //CONFIG_ENABLE_COUNT_REPORT_RATE

#ifdef CONFIG_ENABLE_GLOVE_MODE
unsigned char g_IsEnableGloveMode = 0;
#endif //CONFIG_ENABLE_GLOVE_MODE

unsigned char g_FwData[MAX_UPDATE_FIRMWARE_BUFFER_SIZE][1024];
unsigned int g_FwDataCount = 0;

unsigned char g_IsHotknotEnabled = 0;
unsigned char g_IsBypassHotknot = 0;
...
static signed int _DrvMainCreateProcfsDirEntry(void);
#ifdef CONFIG_ENABLE_HOTKNOT
static signed int _DrvMainHotknotRegistry(void);
#endif //CONFIG_ENABLE_HOTKNOT
...
{
    unsigned int nNum = nCount / 1024;
    unsigned int nRemainder = nCount % 1024;
    unsigned int i;
...
{
    unsigned int i, nLength = 0;
    unsigned char nBank, nAddr;
    unsigned short szRegData[MAX_DEBUG_REGISTER_NUM] = {0};
    unsigned char szOut[MAX_DEBUG_REGISTER_NUM*25] = {0}, szValue[10] = {0};
...
{
    unsigned int i, nLength = 0;
    unsigned char szSmBusRxData[MAX_I2C_TRANSACTION_LENGTH_LIMIT] = {0};
    unsigned char szOut[MAX_I2C_TRANSACTION_LENGTH_LIMIT*2] = {0};
    unsigned char szValue[10] = {0};
...
ssize_t DrvMainProcfsFirmwareSetDQMemValueRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    unsigned int i, nLength = 0;
    unsigned char nBank, nAddr;
    unsigned int szRegData[MAX_DEBUG_REGISTER_NUM] = {0};
    unsigned char szOut[MAX_DEBUG_REGISTER_NUM*25] = {0}, szValue[10] = {0};
...
ssize_t DrvMainProcfsFirmwareSetDQMemValueWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    char *pCh = NULL;
    char *pStr = NULL;
    unsigned short nRealDQMemAddr = 0;
    unsigned int nRealDQMemValue = 0;
...
{    
    unsigned int i;
    unsigned int nFeature = 0, nNewValue = 0;
    char *pCh = NULL;
    char *pStr = NULL;  
...
{
    unsigned char szOut[GESTURE_WAKEUP_INFORMATION_PACKET_LENGTH*5] = {0}, szValue[10] = {0};
    unsigned int szLogGestureInfo[GESTURE_WAKEUP_INFORMATION_PACKET_LENGTH] = {0};
    unsigned int i = 0;
    unsigned int nLength = 0;
...
signed int DrvMainTouchDeviceInitialize(void)
{
    ...
    if (g_ChipType != 0) // To make sure TP is attached on cell phone.
    {
...
#ifdef CONFIG_ENABLE_CHARGER_DETECTION 
        {
            ...
            unsigned char szChargerStatus[20] = {0};
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
    unsigned char *pGesturePath = NULL;
#endif //CONFIG_ENABLE_GESTURE_DEBUG_MODE
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
    unsigned char *pDevicePath = NULL;
