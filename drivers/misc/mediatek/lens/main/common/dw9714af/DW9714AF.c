...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	char puSendCmd[2] = { (char)(a_u2Data >> 4), (char)(((a_u2Data & 0xF) << 4)) + 0x07 };
	...
	if (i4RetValue < 0) {
		//LOG_INF("@@fanfan s4AF_WriteReg() I2C send failed!!\n");
		return -1;
	}
	else{
	//LOG_INF("@@fanfan s4AF_WriteReg() I2C write %d,i4RetValue=%d\n",a_u2Data,i4RetValue);
	}
	return 0;
}
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		/*
		char puSendCmdreset1[2] = { (char)(0x80), (char)(0x00) };
		char puSendCmdreset2[2] = { (char)(0x00), (char)(0x00) };
		char puSendCmd1[2] = { (char)(0xEC), (char)(0xA3) };//                                               
		char puSendCmd2[2] = { (char)(0xA1), (char)(0x05) };//  0x0E for DLC   ||   0x0D for NEW DLC    ||   0x05 for LSC
		char puSendCmd3[2] = { (char)(0xF2), (char)(0xA8) };//  0xA0 for DLC   ||   0x20 for NEW DLC    ||   0xA8 for LSC
		char puSendCmd4[2] = { (char)(0xDC), (char)(0x51) };
		g_pstAF_I2Cclient->addr = AF_I2C_SLAVE_ADDR;
		g_pstAF_I2Cclient->addr = g_pstAF_I2Cclient->addr >> 1;
		i2c_master_send(g_pstAF_I2Cclient, puSendCmdreset1, 2);
		i2c_master_send(g_pstAF_I2Cclient, puSendCmdreset2, 2);
		mdelay(12);
...
#if 0
		while((g_u4TargetPosition < g_u4CurrPosition)&&((g_u4CurrPosition - g_u4TargetPosition) > 25))
		{
			g_u4CurrPosition = g_u4CurrPosition - 25;
