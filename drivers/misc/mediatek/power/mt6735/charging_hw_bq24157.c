...
*/
 ...
 static u32 charging_hw_init(void *data)
 {
  ...
  //gpio_direction_output(charger_enable_pin, 0);
	...
	bq24157_config_interface_liao(0x00,0xC0);	//kick chip watch dog
#ifndef CONFIG_HL7006	
	bq24157_config_interface_liao(0x01,0xb8);	//TE=1, CE=0, HZ_MODE=0, OPA_MODE=0
	bq24157_config_interface_liao(0x05,0x03);
    
	bq24157_config_interface_liao(0x04,0x1A); //146mA
#else
	bq24157_config_interface_liao(0x01,0xF8);	//TE=1, CE=0, HZ_MODE=0, OPA_MODE=0
	bq24157_config_interface_liao(0x05,0x04);
	//bq24158_config_interface_liao(0x04,0x7A); //146mA
	bq24157_set_iterm(0x2);//150ma
#endif    
 	...
 	//MDELAY(10);
	    gpio_set_value(charger_enable_pin, 1);
			...
			if (charging_get_error_state()) {
				bq24157_set_ce(0x1);
				battery_xlog_printk(BAT_LOG_CRTI,"[charging_enable] bq24157_set_hz_mode(0x1)\n");
				bq24157_set_hz_mode(0x1);	// disable power path
			}
		}
	}
	return status;
