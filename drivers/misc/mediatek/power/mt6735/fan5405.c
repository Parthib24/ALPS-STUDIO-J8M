...
static struct i2c_driver fan5405_driver = {
    .driver = {
        .name    = "fan5405",
	 .of_match_table = accel_of_match,//rl add
    },
...
kal_uint8 g_reg_value_fan5405_reg0x06=0;//calm read reg0x06
...
void fan5405_dump_register(void)
{
    ...
    for (i=0;i<fan5405_REG_NUM;i++)
    {
		...
		//start == calm; read reg0x06
		if(i == 6)
		{
		    if(fan5405_reg[i] == 0x5a || fan5405_reg[i] == 0x7a)
			g_reg_value_fan5405_reg0x06 = 1;
...
//start == calm; read reg0x06
static ssize_t show_fan5405_reg0x06(struct device *dev,struct device_attribute *attr, char *buf)
{
	battery_log(BAT_LOG_FULL,"[show_fan5405_reg0x06] 0x%x\n", g_reg_value_fan5405_reg0x06);
...
static DEVICE_ATTR(fan5405_reg0x06, 0664, show_fan5405_reg0x06, NULL); //664//calm read reg0x06
...
{    
    ...
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_fan5405_reg0x06);//clam read reg0x06
    return 0;
}
...
subsys_initcall(fan5405_subsys_init);//daizhiling modify because of reboot
