...
{
		APS_FUNC();
		//add by xujiwei-----2017-2-18------start
		u8 buf[2];
		//add by xujiwei---end
    struct epl_sensor_priv *obj = epl_sensor_obj;
...
static int ps_enable_nodata(int en)
{
	...
	//add by xujiwei-----2017-2-18---start
	bool enable_ps = test_bit(CMC_BIT_PS, &epl_sensor_obj->enable);
	//add by xujiwei-----end
	APS_LOG("enable_ps=%d,en=%d\n",enable_ps,en);
	if(enable_ps!=en){//add by xujiwei-----2017-2-18----start
		mutex_lock(&sensor_mutex);
		APS_LOG("enable_ps!=en\n");
		if (en){
			set_bit(CMC_BIT_PS, &epl_sensor_obj->enable);
			//add by xujiwei----start
            write_global_variable(epl_sensor_obj->client);
			#if PS_DYN_K
            set_psensor_intr_threshold(65534, 65535);   //dont use first ps status
            dynk_min_ps_raw_data = 0xffff;
