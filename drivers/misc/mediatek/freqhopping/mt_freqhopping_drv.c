...
static int freqhopping_mempll_proc_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n",(g_fh_drv_pll[2].fh_status == FH_PLL_ENABLE)?"on":"off");
	return 0;
}
...
static int freqhopping_mempll_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, freqhopping_mempll_proc_read, NULL);
}
...
static const struct file_operations mempll_fops = {
	.owner = THIS_MODULE,
	.open = freqhopping_mempll_proc_open,
	.read = seq_read,
	//.write = seq_write,
	.release = single_release,
};
...
static int freqhopping_debug_proc_init(void)
{
	...
	struct proc_dir_entry *prMempllEntry;
	...
	prMempllEntry =
	    proc_create("rgk_mem_pll", 0444, NULL, &mempll_fops);
	if (prMempllEntry) {
		/* prUserdefEntry->read_proc  = freqhopping_userdefine_proc_read; */
		/* prUserdefEntry->write_proc = freqhopping_userdefine_proc_write; */
		FH_MSG("[%s]: successfully create /proc/prMempllEntry", __func__);
	} else {
		FH_MSG("[%s]: failed to create /proc/prMempllEntry", __func__);
		return 1;
	}
