...
#ifdef CONFIG_MTK_PSC5415A_SUPPORT
extern int start_flag ;

static DECLARE_WAIT_QUEUE_HEAD(psc5415a_int_status_wq);
static atomic_t isOTGstatus= ATOMIC_INIT(0);
#endif
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on) {
#ifdef CONFIG_MTK_PSC5415A_SUPPORT
		    atomic_set(&isOTGstatus, 1);
			 wake_up(&psc5415a_int_status_wq);
#endif
		...
		#elif defined(CONFIG_MTK_PSC5415A_SUPPORT)
  ...
  atomic_set(&isOTGstatus, 0);
    ...
    bq24157_config_interface_liao(0x02,0x8e); 
	  	#elif defined(CONFIG_MTK_PSC5415A_SUPPORT)		
  	  psc5415a_config_interface_liao(0x01,0x30);
	  psc5415a_config_interface_liao(0x02,0x8e); //reset
	#elif defined(CONFIG_MTK_BQ24261_SUPPORT)
...
#if defined(CONFIG_MTK_PSC5415A_SUPPORT)
 static int psc5415a_int_status_kthread(void *x)
{
	 while(1)
	 {	
	 	  // msleep(500);
	 wait_event(psc5415a_int_status_wq, atomic_read(&isOTGstatus));
        //atomic_set(&isOTGstatus, 0);

	  if(start_flag == 1)
	 	   {
	 	     if(0x0 ==  psc5415a_get_opa_mode())
	 	   	 	psc5415a_set_opa_mode(1);
		     else
			 	msleep(500);
	  	 }
...
#endif
#ifdef CONFIG_MTK_PSC5415A_SUPPORT
    kthread_run(psc5415a_int_status_kthread, NULL, "psc5415a_int_status_kthread"); 
#endif
