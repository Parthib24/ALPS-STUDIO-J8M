...
static int MXC400X_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	msleep(MXC400X_STABLE_DELAY);//M by shubin for G-sensor delay MTKcase ALPS03259618
	databuf[0] = MXC400X_REG_CTRL;
	while (i++ < 3)
	{
		res = i2c_master_send(client, databuf, 0x2);
		//msleep(5);//M by shubin for G-sensor delay MTKcase ALPS03259618
		if(res > 0)
			break;
        msleep(5);//A by shubin for G-sensor delay MTKcase ALPS03259618
	}
	...
	delay_state = enable;
#else
	msleep(300);//M by shubin for G-sensor delay MTKcase ALPS03259618
#endif
	return MXC400X_SUCCESS;
}
...
#if 0
	node = of_find_compatible_node(NULL, NULL, "mediatek,pcb_id_info");
	pcb_id1 = of_get_named_gpio(node, "pcb_id1", 0);
	gpio_request(pcb_id1, "pcb_id1");
	gpio_direction_input(pcb_id1);
	msleep(10);
	id1_pin = gpio_get_value(pcb_id1);
	
	if(id1_pin == 1)
	{
		hw->direction = 3;
		GSE_ERR("Used new layout Gsensor\n");
	}
	else
	{
		GSE_ERR("Used old layout Gsensor do not change direction\n");
	}
#endif	
