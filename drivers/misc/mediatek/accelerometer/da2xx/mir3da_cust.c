...
{    
    ...
    else
    {
      ...
      if(bzstk)
		...
		for(i=0; i<MIR3DA_AXES_NUM; i++){
			acc[i] = wispe_filter(acc[i]);
		}
...
int wispe_filter(int num){
	if(num>(0-WISPE_THRES) && num<(0+WISPE_THRES)){
		num = num/WISPE_THRES*WISPE_PARA;
	}else if(num>(GRAVITY_NUMBER-WISPE_THRES) && num<(GRAVITY_NUMBER+WISPE_THRES)){
		num = (num-GRAVITY_NUMBER)/WISPE_THRES*WISPE_PARA+GRAVITY_NUMBER;
	}else if(num<(-GRAVITY_NUMBER+WISPE_THRES) && num>(-GRAVITY_NUMBER-WISPE_THRES)){
		num = -GRAVITY_NUMBER+(GRAVITY_NUMBER+num)/WISPE_THRES*WISPE_PARA;
	}
	return num;
}
/*----------------------------------------------------------------------------*/
...
static const struct of_device_id accel_of_match[] = {
	#if 0	// kcm modify by phf, 20160614, gsensor ¼æÈÝ
