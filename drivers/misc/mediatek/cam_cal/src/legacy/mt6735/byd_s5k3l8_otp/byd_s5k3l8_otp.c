...
//#define CAM_CALDB(fmt, arg...)     xlog_printk(ANDROID_LOG_DEBUG  ,"[byd_s5k3l8XXBYD_EEPROM]" , fmt, ##arg)//printk
#if 1
#define CAM_CALGETDLT_DEBUG
#define CAM_CAL_DEBUG
#ifdef CAM_CAL_DEBUG
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "BYD_CAM_CAL_DRV"
...
{
 CAM_CALDB("@xiaofei_[byd_s5k3l8XXBYD_CAM_CALL][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
    int  i4RetValue = 0;
    ...
    if (i4RetValue != 2)
    {
        CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] I2C send read address failed!! \n");
        return -1;
    }
    i4RetValue = i2c_master_recv(g_pstI2Cclient, (char *)a_puBuff, ui4_length);
	
	CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL][iReadCAM_CAL] Read 0x%x=0x%x \n", a_u2Addr, a_puBuff[0]);
    
    if (i4RetValue != ui4_length)
    {
        CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] I2C read data failed!! \n");
        return -1;
    } 
...
static int Read_LSC_Data(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   char *readbuff;
   int RetValue;
   int i;
readbuff = pinputdata;
	printk("[wgs] Enter into Read_LSC_Data\n");
	for(i=0; i<ui4_length; i++)
 {   

	       RetValue = iReadCAM_CAL((u16)ui4_offset+i, 1, pinputdata+i);
	       if (RetValue != 0)
	       {
		    		 CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] [Read_LSC_Data]I2C iReadData failed!! \n");
		    			return -1;
	       }   
    

	      CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL][Read_LSC_Data] Read lsc data[%d]=0x%x \n", i, readbuff[i]);
 }
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   u4CurrentOffset = ui4_offset;
   pBuff = pinputdata;
//rewritten by wrj 2015.8.19
#if 1
   if(ui4_length == 1868)//LSC
   {
   ...
   else if(ui4_length ==8)//AWB
    {
	      ...
	      u8 readbuff, i;
			...
			for(i=0;i<ui4_length;i++)
			{
	    	iReadCAM_CAL((u16)u4CurrentOffset+i, 1, &readbuff);
				*(pBuff+i)=readbuff;
		    CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL_wgs][iReadData] read awb data[%d]=0x%x\n",i,pBuff[i]);
			}
	     
			//return KAL_TRUE;
	   	
    }
	  else if(ui4_length ==4)//AF
    {
	  	    u8 readbuff, i;
	        ...
	        iReadCAM_CAL((u16)u4CurrentOffset+i, 1, &readbuff);
					*(pBuff+i)=readbuff;
		      CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL][iReadData] read af data[%d]=0x%x\n",i,pBuff[i]);
			}
   ...
   do 
   {
   	   printk("Enter into 04\n");
       i4RetValue = iReadCAM_CAL((u16)u4CurrentOffset, 1, pBuff);
	   CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL][iReadData] Read 0x%x=0x%x \n", u4CurrentOffset, pBuff[0]);
       if (i4RetValue != 0)
       {
            CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] I2C iReadData failed!! \n");
            return -1;
       }           
       u4IncOffset++;
       i4ResidueDataLength --;
...
static long byd_s5k3l8_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
...
#endif
{
	...
	CAM_CALDB("@xiaofei_into Ioctl\n");
	printk("@xiaofei_into Ioctl\n");
	...
	CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] a_u4Command 0x%x \n", a_u4Command);
	printk("[byd_s5k3l8XXBYD_CAM_CALL] a_u4Command 0x%x \n", a_u4Command);
    switch(a_u4Command)
    {
	    ...
	    printk("Enter into CAM_CALIOC_G_READ\n");
          CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] Read CMD \n");
	    #if 0		//rewritten by wrj 2015.8.19
	    if(ptempbuf->u4Offset == 4)
		ptempbuf->u4Offset = 1;
            #endif

	    otp_flag_byd = 1;	//rewritten by wrj 2015.8.19

            CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] offset 0x%x \n", ptempbuf->u4Offset);
            CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] length %d \n", ptempbuf->u4Length);
      //CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] Before read Working buffer address 0x%llx \n", (u32)pWorkingBuff);
	    #if 1		//rewritten by wrj 2015.8.19
            ...
            CAM_CALDB("[byd_s5k3l8XXBYD_CAM_CALL] After read Working buffer data  0x%8x \n", *pWorkingBuff);
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = byd_s5k3l8_Ioctl_Compat,
#endif
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = BYD_S5K3L8_OTP_DEVICE_ID >> 1;
