...
static int  mt6797_dts_gpio_init(struct spi_device**   spidev)
{
    ...
    spi->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6735m-finger");
...
int mt6797_pinctrl_realease(struct spi_device**    spi)
{
	
	if(spi==NULL ||(*spi)==NULL || (*spi)->dev.of_node==NULL || this_pctrl == NULL){
		return -1;	
	}
	devm_pinctrl_put(this_pctrl);
	return 0;	
}
EXPORT_SYMBOL_GPL(mt6797_pinctrl_realease);
