...
{
#ifdef CONFIG_FINGERPRINT_SW9651	
#pragma message("------------------------CONFIG_FINGERPRINT_SW9651")
	seq_printf(m, "sw9651\n" );	
#else
	seq_printf(m, "sw9562\n" );	
#endif
...
sunwave_dev_read(struct file* filp, char __user* buf, size_t count, loff_t* f_pos)
{
    ...
    if (sunwave_dev->finger && sunwave_dev->finger->write_then_read == 0) {
        ...
        if (status >= 0) {
            ...
            sw_dbg("ret %x missing = %d status = %d", (buf[status - 2] << 8) | buf[status - 1], missing, status);
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
	...
	{			
	 spi->dev.of_node = of_find_compatible_node(NULL,NULL,"sunwave,mt6735m-finger");
	}
	spi->bits_per_word = 8;
	#endif
    ...
    if (sunwave_dev->finger && sunwave_dev->finger->init) {		
        ...
        if (status < 0) {
            ...
            mt6797_pinctrl_realease(&sunwave_dev->spi);
            return status;
        }
    }
sw_power_init(spi);
