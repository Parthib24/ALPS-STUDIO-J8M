...
int gf_parse_dts(struct gf_dev* pdev)
{
	...
	if(pdev->spi->dev.of_node == NULL) {
        pdev->spi->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6735m-finger");
	}
    ...
    if(pdev->spi->dev.of_node) {
    	...
    	pdev->fp_rst_high = pinctrl_lookup_state(pdev->pinctrl1, "fingerprint_reset_high");
    	if (IS_ERR(pdev->fp_rst_high)) {
    		ret = PTR_ERR(pdev->fp_rst_high);
    		dev_err(&pdev->spi->dev, " Cannot find fp pinctrl fingerprint_reset_high!\n");
    		return ret;
    	}
    	pdev->fp_rst_low = pinctrl_lookup_state(pdev->pinctrl1, "fingerprint_reset_low");
    	if (IS_ERR(pdev->fp_rst_low)) {
    		ret = PTR_ERR(pdev->fp_rst_low);
    		dev_err(&pdev->spi->dev, " Cannot find fp pinctrl fingerprint_reset_low!\n");
    		return ret;
    	}
    	pdev->eint_as_int = pinctrl_lookup_state(pdev->pinctrl1, "fingerprint_pin_irq");
...
int gf_power_init(struct gf_dev* pdev) 
{
    ...
    FUNC_ENTRY();
    pdev->avdd = regulator_get(&pdev->spi->dev, "vgp1");
    if(IS_ERR(pdev->avdd)) {
