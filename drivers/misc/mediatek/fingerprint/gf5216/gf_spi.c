...
int gf_irq_num(struct gf_dev *pdev)
{
	...
	FUNC_ENTRY();
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt6735m-finger"); //lsm modify
	if(node) {
		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_request(ints[0], "fingerprint-eint");
...
int rgk_creat_proc_fingerprint_info(void)
{
...
static int gf_probe(struct spi_device *spi)
{
   ...
   if (gf_hw_reset(gf_dev, 0))
	...
	/******************************************************/
	sysfs_create_group(&spi->dev.kobj,&gf_debug_attr_group);
  /******************************************************/
