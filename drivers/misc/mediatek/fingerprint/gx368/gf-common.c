				...
				u16 addr, u32 data_len, u8 *rx_buf)
{
	...
	if((package_num > 0) && (reminder != 0)) {
		twice = 1;
		gf_debug(FLOW_DEBUG,"stone package_num is %d reminder is %d\n",package_num,reminder);
	} else {
		twice = 0;
	}
	if( xfer == NULL){
...
int gf_spi_read_data(gf_dev_t* gf_dev, u16 addr, int len, u8* value)
{
	...
	if(len>=1022)
	{
		gf_debug(FLOW_DEBUG,"stone value[len-5]=%x,%x,%x,%x,%x",value[len-5],value[len-4],value[len-3],value[len-2],value[len-1]);
	}
    mutex_unlock(&gf_dev->buf_lock);	
