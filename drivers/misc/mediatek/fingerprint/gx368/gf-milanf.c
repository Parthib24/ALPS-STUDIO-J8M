...
#define FINGERPRINT_USING_EXT_LDO   //add by xujiwei-----2016-10-8 
...
static struct pinctrl_state *eint_as_int, *rst_output0, *rst_output1, *eint_pulldown,*ext_ldo_en0,*ext_ldo_en1;
static int compatible_GF368M = 0;
struct of_device_id fingerprint_of_match[] = {
	{ .compatible = "mediatek,mt6735m-finger", },
	{},
};
...
{
	int ret = 0;
#if 0 	 //modify by xuliu to remove the power on and power down 
	struct regulator *reg = NULL;
	
	gf_debug(DEFAULT_DEBUG,"%s onoff = %d", __func__, onoff);
	
	reg = regulator_get(&gf_dev->spi->dev, "vfingerprint");
	ret = regulator_set_voltage(reg, 2800000, 2800000);
	if (ret) {
		gf_error("regulator_set_voltage(%d) failed!\n", ret);
	}
#endif	
	if(onoff) {
		if (0 == gf_dev->poweron){
			gf_debug(DEFAULT_DEBUG, "power on %s\n", __func__);
			ret = pinctrl_select_state(pinctrl1, rst_output0);
			if(ret){
				gf_error("[%s]pinctrl_select_state rst_output0 error",__func__);
			}
			ret = pinctrl_select_state(pinctrl1, eint_pulldown);
			if(ret){
				gf_error("[%s]pinctrl_select_state eint_pulldown error",__func__);
			}
			mdelay(5);
#ifdef FINGERPRINT_USING_EXT_LDO			
			ret = pinctrl_select_state(pinctrl1, ext_ldo_en1);
			if(ret){
				gf_error("[%s]pinctrl_select_state ext_ldo_en1 error",__func__);
			}
#endif			
#if 0			 	 //modify by xuliu to remove the power on and power down 
			ret = regulator_enable(reg);	
			if (ret)
				gf_error("regulator_enable() failed!\n");
#endif				
			ret = pinctrl_select_state(pinctrl1, eint_as_int);
			if(ret){
				gf_error("[%s]pinctrl_select_state eint_as_int error",__func__);
			}
			msleep(10);
			ret = pinctrl_select_state(pinctrl1, rst_output1);
			if(ret){
				gf_error("[%s]pinctrl_select_state rst_output1 error",__func__);
			}
			gf_dev->poweron = 1;
		}
	} else {
		if (1 == gf_dev->poweron){
			gf_debug(DEFAULT_DEBUG, "power off %s\n", __func__);
			ret = pinctrl_select_state(pinctrl1, rst_output0);
			if(ret){
				gf_error("[%s]pinctrl_select_state rst_output0 error",__func__);
			}
#ifdef FINGERPRINT_USING_EXT_LDO			
			ret = pinctrl_select_state(pinctrl1, ext_ldo_en0);
			if(ret){
				gf_error("[%s]pinctrl_select_state ext_ldo_en0 error",__func__);
			}
#endif			
#if 0			 	 //modify by xuliu to remove the power on and power down 
			ret = regulator_disable(reg);
			if (ret)
				gf_error("regulator_disable() failed!\n");
#endif				
			gf_dev->poweron = 0;
			msleep(2);
		}
...
static unsigned short Fw_version[2] = {0};

static ssize_t gx368_check_chipID_show(struct device *dev, 
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%04x.0x%04x", Fw_version[0],Fw_version[1]);
	
}

static ssize_t gx368_check_chipID_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
        gf_debug(SPI_DEBUG,"gx368:gx368_check_chipID_store\n");
	return count;
}

static DEVICE_ATTR(debug, S_IRUGO|S_IWUSR, gf_debug_show, gf_debug_store);
static DEVICE_ATTR(chipId, S_IRUGO|S_IWUSR, gx368_check_chipID_show, gx368_check_chipID_store);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_SENDKEY:
		...
		for(i = 0; i< ARRAY_SIZE(key_map); i++) {
				gf_error("gf key_map[%d].val = %d, gf_key.key = %d.", i, key_map[i].val, gf_key.key);
...
{
	gf_dev_t *gf_dev = &gf;
	gf_debug(DEFAULT_DEBUG, "DHY %s is entry!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",__func__);
#if GF_FASYNC
	if(gf_dev->async) {
...
#endif
	gf_debug(DEFAULT_DEBUG, "DHY %s will exit!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!",__func__);
	return IRQ_HANDLED;
    ...
    if(status == 0){
		...
		if(status == 0) {
			...
			while(cnt < 10){
                ...
                pr_info("%s IRQ status : 0x%x ,cnt = %d\n",__func__,reg,cnt);
                if(reg == 0x0100) {
...
int fingerprint_get_gpio_info(struct spi_device *pdev)
{
	int ret;
	...
	pdev->dev.of_node = of_find_compatible_node(NULL, NULL,"mediatek,mt6735m-finger");
	if(!(pdev->dev.of_node)){
		gf_error("of_find_compatible_node error\n");
		return -1;
	}
	gf_debug(FLOW_DEBUG,"of_find_compatible_node ok\n");
	pinctrl1 = devm_pinctrl_get(&pdev->dev);
	if(pinctrl1 == NULL)
	{
		gf_error(" pinctrl1 = NULL \n");
		return -1;
	}
	if (IS_ERR(pinctrl1)) {
		ret = PTR_ERR(pinctrl1);
		gf_error(" .ret = %d\n",ret);
		dev_err(&pdev->dev, "fwq Cannot find fingerprint pinctrl1!\n");
		return ret;
	}
	pins_default = pinctrl_lookup_state(pinctrl1, "fingerprint_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		dev_err(&pdev->dev, "fwq Cannot find fingerprint pinctrl fingerprint_default %d!\n", ret);
	}
	eint_as_int = pinctrl_lookup_state(pinctrl1, "fingerprint_pin_irq");
	if (IS_ERR(eint_as_int)) {
		ret = PTR_ERR(eint_as_int);
		dev_err(&pdev->dev, "fwq Cannot find fingerprint pinctrl fingerprint_pin_irq!\n");
		return ret;
	}
	eint_pulldown = pinctrl_lookup_state(pinctrl1, "fingerprint_pin_irq_pd");
	if (IS_ERR(eint_pulldown)) {
		ret = PTR_ERR(eint_pulldown);
		dev_err(&pdev->dev, "fwq Cannot find fingerprint pinctrl fingerprint_pin_irq_pd!\n");
		return ret;
	}
	rst_output0 = pinctrl_lookup_state(pinctrl1, "fingerprint_reset_low");
	if (IS_ERR(rst_output0)) {
		ret = PTR_ERR(rst_output0);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_reset_low!\n");
		return ret;
	}
	rst_output1 = pinctrl_lookup_state(pinctrl1, "fingerprint_reset_high");
	if (IS_ERR(rst_output1)) {
		ret = PTR_ERR(rst_output1);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_reset_high!\n");
		return ret;
	}
#ifdef FINGERPRINT_USING_EXT_LDO	
	ext_ldo_en0 = pinctrl_lookup_state(pinctrl1, "fingerprint_ext_ldo_en_low");
	if (IS_ERR(ext_ldo_en0)) {
		ret = PTR_ERR(ext_ldo_en0);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_ext_ldo_en_low!\n");
		return ret;
	}
	ext_ldo_en1 = pinctrl_lookup_state(pinctrl1, "fingerprint_ext_ldo_en_high");
	if (IS_ERR(ext_ldo_en1)) {
		ret = PTR_ERR(ext_ldo_en1);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_ext_ldo_en_high!\n");
		return ret;
	}
#endif	
#if 0
	power_output0 = pinctrl_lookup_state(pinctrl1, "fingerprint_power_low");
	if (IS_ERR(power_output0)) {
		ret = PTR_ERR(power_output0);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_power_low!\n");
		return ret;
	}
	power_output1 = pinctrl_lookup_state(pinctrl1, "fingerprint_power_high");
	if (IS_ERR(power_output1)) {
		ret = PTR_ERR(power_output1);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl fingerprint_power_high!\n");
		return ret;
	}
	...
	if (IS_ERR(eint_pulldown)) {
		ret = PTR_ERR(eint_pulldown);
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl state_eint_pulldown!\n");
		return ret;
	}
	...
	if (IS_ERR(eint_pulldisable)) {
		...
		dev_err(&pdev->dev, "fwq Cannot find  ngerprint pinctrl state_eint_pulldisable!\n");
		return ret;
	}
#endif
	gf_debug(FLOW_DEBUG,"[fingerprint] mt_fingerprint_pinctrl ok----------\n");
	return 0;
}

static int pinctrl_realease(void){
	if(gf.spi==NULL || 	gf.spi->dev.of_node==NULL ||pinctrl1 == NULL){
		return -1;	
	}
...
static int  gf_probe(struct spi_device *spi)
{
	...
	FUNC_ENTRY();
	gf_debug(FLOW_DEBUG,"driver version is %s.\n",GF_DRIVER_VERSION);
	...
	ret = pinctrl_select_state(pinctrl1, eint_as_int);
	if(ret){
		gf_debug(FLOW_DEBUG,"pinctrl_select_state eint_as_int error\n");
	}
	...
	for(tt=0;tt<5;tt++) {
			chip_id_1 = 0;
			chip_id_2 = 0;
		gf_debug(FLOW_DEBUG," %s %d\n",__func__,__LINE__);

		gf_spi_read_word(gf_dev,0x0000,&chip_id_1);
		gf_spi_read_word(gf_dev,0x0002,&chip_id_2);
		if(chip_id_2 == 0x0022 && chip_id_1 == 0x02a0 ){
			printk(KERN_ERR"[%s-%d]compatible driver ok\n",__func__,__LINE__);
			compatible_GF368M = 1;
			//app_get_fingerprint_name("GF368M");
			break;
		}
		gf_debug(FLOW_DEBUG,"%s chip id is 0x%04x 0x%04x \n",__func__,chip_id_2,chip_id_1);
	}
	if(tt==5){
		printk(KERN_ERR"[%s-%d]compatible driver error\n",__func__,__LINE__);
		compatible_GF368M = 0;
		pinctrl_realease();
		return -1;
	} else {
		Fw_version[0] = chip_id_1;
		Fw_version[1] = chip_id_2;
	}
#endif
	...
	//node = of_find_matching_node(node, "mediatek,fingerprint-eint");
