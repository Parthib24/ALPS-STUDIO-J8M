...
//rgk xuliu add for *#*#9375#*#* start (2016-12-1)
#include <linux/seq_file.h>
#include <linux/proc_fs.h> 
//rgk xuliu add for *#*#9375#*#* end (2016-12-1)
...
#define GF_INPUT_NAME "goodix_fp_btn"
...
#define	GF_KEY_DOUBLETAP	KEY_F12
/**************************debug******************************/
#define GF_DEBUG
/*#undef  GF_DEBUG*/
...
static ssize_t gx368_check_chipID_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
        gf_dbg("gx368:gx368_check_chipID_store\n");
	return count;
}
                     ...
                     unsigned long arg)
{
    ...
    case GF_IOC_SENDKEY:
		...
		if(gf_key.key==KEY_BACK){
			gf_key.key = GF_KEY_BACK; 	
		}
		else if(gf_key.key==KEY_F11){
			gf_key.key = 	GF_KEY_DOUBLETAP;
		}
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
    ...
    __set_bit(GF_KEY_F3, gf_dev->input->keybit);
		...
		__set_bit(GF_KEY_DOUBLETAP, gf_dev->input->keybit);
...
#endif
{
    ...
    if (minor < N_SPI_MINORS) {
		...
		if(status){
		    gf_dbg("Failed to create sysfs file.");
		    goto error;
		}
...
#pragma message("--------------------------------USE_PLATFORM_BUS")
    status = platform_driver_register(&gf_driver);
#elif defined(USE_SPI_BUS)
#pragma message("--------------------------------USE_SPI_BUS")
