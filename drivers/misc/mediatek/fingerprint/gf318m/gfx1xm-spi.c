...
#define GFX1XM_X18M_TARGET_FW
#define GFX1XM_318M_TARGET_CFG

#if defined GFX1XM_X16M_TARGET_FW
#define GFX1XM_PID "GFx16M"
#define GFX1XM_NAV_FRAME_LEN         (1680)    //navigation data
#elif defined GFX1XM_X18M_TARGET_FW
#define GFX1XM_PID "GFx18M"
#define GFX1XM_NAV_FRAME_LEN        (2160)     //navigation data len
#else
#error "have't define any product ID"
#endif
#define GFX1XM_PID_LEN 6

static unsigned char Fw_version[16] = {0};


struct gfx1xm_key_map
{
    char *name;
    unsigned short val;
};

struct gfx1xm_key_map key_map[] = 
{
      {  "POWER",  KEY_POWER  },
      {  "HOME" ,  KEY_F11   },
      {  "MENU" ,  KEY_MENU   },
...
#define GFX1XM_INPUT_HOME_KEY   KEY_F11//KEY_HOME//KEY_F11
#define GFX1XM_FF_KEY           KEY_F10//KEY_POWER
#if (NAVIGATION == 1)
#define GFX1XM_UP_KEY       KEY_UP
#define GFX1XM_DOWN_KEY KEY_DOWN
#define GFX1XM_LEFT_KEY KEY_LEFT
#define GFX1XM_RIGHT_KEY    KEY_RIGHT
#define GFX1XM_KEY_FORCE    KEY_F9
#define GFX1XM_APP_SWITCH   KEY_F19
#endif
...
{
pr_warn("---------gfx1xm_power_on----entry--\n");
#if 1
	if(g_ReguVdd==NULL)		
	{			
		g_ReguVdd = regulator_get(&gfx1xm.spi->dev, vdd_name);			
		regulator_set_voltage(g_ReguVdd, 2800000, 2800000); 		
	}	
	if(onoff)	
	{			
		    if (regulator_enable(g_ReguVdd))
		    {
		        pr_warn("gfx1xm_power_on 2800mv error.\n");
		    }
		   else
		   	{
		   		pr_warn("gfx1xm_power_on 2800mv ok.\n");
		   	}
	}	
	else
	{		
...
static ssize_t gfx1xm_check_chipID_show(struct device *dev, 
	struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%c%c%c%c%c%c%c%x.%02x.%02x", Fw_version[0],Fw_version[1],Fw_version[2],Fw_version[3],Fw_version[4],Fw_version[5],Fw_version[6],Fw_version[7],Fw_version[8],Fw_version[9]);
	
}

static ssize_t gfx1xm_check_chipID_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
        gfx1xm_debug(SPI_DEBUG,"gfx1xm:gfx1xm_check_chipID_store\n");
	return count;
}
static DEVICE_ATTR(debug, S_IRUGO|S_IWUSR, gfx1xm_debug_show, gfx1xm_debug_store);
static DEVICE_ATTR(chipId, S_IRUGO|S_IWUSR, gfx1xm_check_chipID_show, gfx1xm_check_chipID_store);
...
int gfx1xm_read_nav_base(struct gfx1xm_dev *gfx1xm_dev, struct gfx1xm_nagv *gfx1xm_nagv)
{
    unsigned char status = 0;
    unsigned char lbstatus = 0;
    int retry = 0;
    int ret = 0;
    if(gfx1xm_nagv->buf == NULL)  {
        gfx1xm_debug(DEFAULT_DEBUG, "get_nav_base is NULL.\n");
        return -EINVAL;
    }


    disable_irq(gfx1xm_dev->spi->irq);
    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, GFX1XM_PRENAV_MODE);
    do
    {
        /* read status and frame num buffered*/
        gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, &status);
        gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_LBSTATUS_ADDR, &lbstatus);
        //pr_info("NAV: retry: %d, status: 0x%x, lbstatus:0x%x\n", retry, status, lbstatus);

        if( (status & GFX1XM_BUF_STA_MASK) && (lbstatus & GFX1XM_NAV_MASK))
        {/* read navigation base */
            mutex_lock(&gfx1xm_dev->buf_lock);
            ret = gfx1xm_spi_read_bytes(gfx1xm_dev, GFX1XM_NAV_BUF0_ADDR, GFX1XM_NAV_FRAME_LEN, gfx1xm_dev->buffer);
            mutex_unlock(&gfx1xm_dev->buf_lock);

            if(!ret) {
                gfx1xm_debug(DEFAULT_DEBUG, "NAV:Failed to read nav base. len = %d. ret = %d\n", GFX1XM_NAV_FRAME_LEN, ret);
            ...
            {
                gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_PAUSE_ADDR, 1);
                if(copy_to_user(gfx1xm_nagv->buf, (gfx1xm_dev->buffer + GFX1XM_RDATA_OFFSET), GFX1XM_NAV_FRAME_LEN)){
    ...
    gfx1xm_debug(DEFAULT_DEBUG, " NAV base out: retry : %d\n", retry);
    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, GFX1XM_IMAGE_MODE);
    enable_irq(gfx1xm_dev->spi->irq);
    return 0;
}

int get_nav_buf(struct gfx1xm_dev *gfx1xm_dev, struct gfx1xm_nagv *gfx1xm_nagv)
{
    unsigned char lbstatus = 0;
    int ret = 0;
    ...
    if(gfx1xm_nagv->buf == NULL) {
        gfx1xm_debug(DEFAULT_DEBUG, "get_package_data buf is NULL.\n");
        return -EINVAL;
    }
    /* read status and frame num buffered*/
    gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_LBSTATUS_ADDR, &lbstatus);
    gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_NAV_FRAMENUM_ADDR, &pNavFrameNum);
    gfx1xm_debug(DEFAULT_DEBUG, "NAV: lbstatus: 0x%x, frame_num:0x%x\n", lbstatus, pNavFrameNum);

    frameNum = pNavFrameNum & 0x0F;
    if(frameNum)
    {/* read navigation data */
        gfx1xm_debug(DEFAULT_DEBUG, "NAV: read %d frame data.\n", frameNum);
        mutex_lock(&gfx1xm_dev->buf_lock);
        gfx1xm_dev->spi->max_speed_hz=4*1000*1000;
		gfx1xm_spi_set_mode(gfx1xm_dev->spi, SPEED_4MHZ, 0);
        if(lbstatus & GFX1XM_NAV_BUF_MASK)
            ret = gfx1xm_spi_read_bytes(gfx1xm_dev, GFX1XM_NAV_BUF1_ADDR, (frameNum*GFX1XM_NAV_FRAME_LEN), gfx1xm_dev->buffer);
        else
            ret = gfx1xm_spi_read_bytes(gfx1xm_dev, GFX1XM_NAV_BUF0_ADDR, (frameNum*GFX1XM_NAV_FRAME_LEN), gfx1xm_dev->buffer);
        ...
        if(!ret) {
            gfx1xm_debug(DEFAULT_DEBUG, "NAV:Failed to read data. len = %d. ret = %d, lbstatus = %d, nav_frm_num = %d\n",
                               GFX1XM_NAV_FRAME_LEN, ret, lbstatus, pNavFrameNum);
                return -EFAULT;
        }
/*
        ...
        checksum16 = checksum((unsigned char*)(gfx1xm_dev->buffer + GFX1XM_RDATA_OFFSET), 
                            (unsigned int)(GFX1XM_NAV_FRAME_LEN*frameNum));
        gfx1xm_spi_read_byte(gfx1xm_dev, 0x9A9A, &ck_r10);
        gfx1xm_spi_read_byte(gfx1xm_dev, 0x9A9B, &ck_r11);
        gfx1xm_spi_read_byte(gfx1xm_dev, 0xB49A, &ck_r20);
        gfx1xm_spi_read_byte(gfx1xm_dev, 0xB49B, &ck_r21);
        pr_info("NAV: checksum:0x%x, ck_r1:[0x%x-0x%x], ck_r2:[0x%x-0x%x].\n", checksum16, ck_r10, ck_r11, ck_r20, ck_r21);
*/     
        if(copy_to_user(gfx1xm_nagv->buf, (gfx1xm_dev->buffer + GFX1XM_RDATA_OFFSET), GFX1XM_NAV_FRAME_LEN*frameNum)){
            pr_err("gf:Failed to copy data from kernel to user.\n");
            return -EFAULT;
        }
        if(copy_to_user(gfx1xm_nagv->frame_num, &pNavFrameNum, 1)){
            pr_err("gf:Failed to copy data from kernel to user.\n");
            return -EFAULT;
        }
    }
    ...
    {
        gfx1xm_debug(DEFAULT_DEBUG, "NAV: no data to read.\n");
    }
 
    /*tell the hardware. Data has been read already.*/
    gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, 0);
    return 0;
}
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    struct gfx1xm_key gfx1xm_key = { 0 };
    struct gfx1xm_nagv gfx1xm_nagv = { 0 };
    int			err = 0;
    u32			tmp = 0;
    int 		retval = 0;
    u8 *temp_buf;
	...
	case GFX1XM_IOC_CMD:
	    ...
	    if(ioc->cmd == GFX1XM_R) {
			...
			if(copy_to_user((void __user*)((unsigned long)ioc->buf), gfx1xm_dev->buffer + GFX1XM_RDATA_OFFSET, ioc->len)) {
			    gfx1xm_error("gf1x1m 64bit Failed to copy data from kernel to user.");
			    retval = -EFAULT;
			    mutex_unlock(&gfx1xm_dev->buf_lock);
				mutex_unlock(&gfx1xm_dev->fb_lock);
			    break;
			...
			if(copy_from_user(gfx1xm_dev->buffer + GFX1XM_WDATA_OFFSET, (void __user*)((unsigned long) ioc->buf), ioc->len)){
			    gfx1xm_error("gf1x1m 32bit Failed to copy data from user to kernel.");
			    retval = -EFAULT;
			    mutex_unlock(&gfx1xm_dev->buf_lock);
				mutex_unlock(&gfx1xm_dev->fb_lock);
			    break;
	...
	case GFX1XM_IOC_REINIT:
		...
		mutex_unlock(&gfx1xm_dev->fb_lock);
	    break;
	 case GFX1XM_IOC_SENDKEY:            //.so define
            if (copy_from_user
                    (&gfx1xm_key, (struct gfx1xm_key *)arg, sizeof(struct gfx1xm_key))) {
            ...
            for(i = 0; i< ARRAY_SIZE(key_map); i++) {
                if(key_map[i].val == gfx1xm_key.key){
                    input_report_key(gfx1xm_dev->input, gfx1xm_key.key, gfx1xm_key.value);
            ...
            if(i == ARRAY_SIZE(key_map)) {
                pr_warn("gfx1xm: key %d not support yet \n", gfx1xm_key.key);
                retval = -EFAULT; 
            } 

            break;
        case GFX1XM_IOC_GET_NAGV_BASE:      //.so define
            if (copy_from_user
                    (&gfx1xm_nagv, (struct gfx1xm_nagv *)arg, sizeof(struct gfx1xm_nagv))) {
                pr_warn("gfx1xm: Failed to copy data from user space.\n");
                retval = -EFAULT;
                break;
            }
            retval = gfx1xm_read_nav_base(gfx1xm_dev, &gfx1xm_nagv);
            break;
        case GFX1XM_IOC_GET_NAGV_DATA:    //.so define
            if (copy_from_user
                    (&gfx1xm_nagv, (struct gfx1xm_nagv *)arg, sizeof(struct gfx1xm_nagv))) {
                pr_warn("gfx1xm: Failed to copy data from user space.\n");
                retval = -EFAULT;
            ...
            retval = get_nav_buf(gfx1xm_dev, &gfx1xm_nagv);
            break;
...
static void gfx1xm_reg_key_kernel(struct gfx1xm_dev *gfx1xm_dev)
{
    int i;

    set_bit(EV_KEY, gfx1xm_dev->input->evbit); //tell the kernel is key event
    for(i = 0; i< ARRAY_SIZE(key_map); i++) {
        set_bit(key_map[i].val, gfx1xm_dev->input->keybit);
    }

    gfx1xm_dev->input->name = GFX1XM_INPUT_NAME;
...
static int gfx1xm_event_handler(void *para)
{
	...
	do{
		...
		case GFX1XM_KEY_MODE:
			#if (NAVIGATION == 1)
            	gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_LBSTATUS_ADDR, &lbstatus); 
            	gfx1xm_debug(DEFAULT_DEBUG, "gfx1xm:Key mode: status = 0x%x lbstatus = 0x%x\n", status, lbstatus);

            	if(lbstatus & GFX1XM_NGKEY_MASK) {
                	if(lbstatus & GFX1XM_NVG_KEY_MASK) { //Navigator is enabled
                    	ngv = (lbstatus & GFX1XM_NVG_KEY_STA) >> 4;
                    	if (NGV_VALUE_UP == ngv) {
                        	gfx1xm_debug(DEFAULT_DEBUG, "UP");
                        	input_report_key(gfx1xm_dev->input, GFX1XM_UP_KEY, 1);
                        	input_sync(gfx1xm_dev->input);
                        	input_report_key(gfx1xm_dev->input, GFX1XM_UP_KEY, 0);
                        	input_sync(gfx1xm_dev->input);
                    	} else if (NGV_VALUE_DOWN == ngv) {
                        	gfx1xm_debug(DEFAULT_DEBUG, "DOWN");
                        	input_report_key(gfx1xm_dev->input, GFX1XM_DOWN_KEY, 1);
                        	input_sync(gfx1xm_dev->input);
                        	input_report_key(gfx1xm_dev->input, GFX1XM_DOWN_KEY, 0);
                        	input_sync(gfx1xm_dev->input);
                    	} else if (NGV_VALUE_LEFT == ngv) {
                        	/*
                        	pr_info("KEY_BACK");
                         	gfx1xm_send_key(KEY_BACK, 1);   
                           	gfx1xm_send_key(KEY_BACK, 0);   
                         	*/
                        	gfx1xm_debug(DEFAULT_DEBUG, "LEFT");
                        	input_report_key(gfx1xm_dev->input, GFX1XM_INPUT_MENU_KEY, 1);
                        	input_sync(gfx1xm_dev->input);
                        	input_report_key(gfx1xm_dev->input, GFX1XM_INPUT_MENU_KEY, 0);
                        	input_sync(gfx1xm_dev->input);
                    	} else if (NGV_VALUE_RIGHT == ngv) {
                        	gfx1xm_debug(DEFAULT_DEBUG, "RIGHT");
                        	/*gfx1xm_send_key(KEY_F17, 1);  
                          	gfx1xm_send_key(KEY_F17, 0);    
                         	*/
                	...
                	if (lbstatus&GFX1XM_FCE_KEY_MASK) { // force value is enabled.
                    	gfx1xm_spi_read_byte(gfx1xm_dev, GFX1XM_FORCE_VALUE_ADDR, &force_val);
                    	gfx1xm_debug(DEFAULT_DEBUG, "FORCE VALUE. force_val : %d\n", force_val);
                    	input_report_key(gfx1xm_dev->input, GFX1XM_KEY_FORCE, force_val);
                    	input_sync(gfx1xm_dev->input);
                	}
            	} else if (status & GFX1XM_KEY_MASK) {
                	gfx1xm_debug(DEFAULT_DEBUG, "input home key %d\n", GFX1XM_INPUT_HOME_KEY);
...
#endif
		    break;
		case GFX1XM_NAV_MODE:
            		gfx1xm_debug(DEFAULT_DEBUG, "Macky:IRQ status = 0x%x, lbstatus = 0x%x, mode = %d\n", status, lbstatus, mode);
            		sendnlmsg("nagv");
            		break;
		default:
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	if(chip_version_config==0x38){		
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			...
			gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: Vendor ID not matched---xielei _4!!!  send default cfg group0\n");
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg8_info_group0, GFX1XM_CFG_LEN); //add by pxm
			//return 0;
		}
	}else if(chip_version_config==0x36){
		if(!memcmp(buffer+GFX1XM_RDATA_OFFSET, vendor_id_1, 1)) {
			...
			gfx1xm_debug(DEFAULT_DEBUG,"gfx1xm: Vendor ID not matched!!! send default cfg group0\n");
			memcpy(gfx1xm_dev->config + GFX1XM_WDATA_OFFSET, cfg6_info_group0, GFX1XM_CFG_LEN); //add by pxm
			//return 0;
		}
	}
...
static int  gfx1xm_probe(struct spi_device *spi)
{
	...
	#if 1
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,mt6753-finger");
...
#error "have't define any product for fw"
	...
	*/
	gfx1xm_reg_key_kernel(gfx1xm_dev);  //key map 
#if FW_UPDATE
	...
	//xielei @20160108
	finger_irq_node= of_find_compatible_node(NULL,NULL, "mediatek,gf518m");
	if(finger_irq_node)
	{
		...
		if(request_irq(gfx1xm_dev->spi->irq,gfx1xm_irq,IRQF_TRIGGER_RISING,"FINGERPRINT-eint",NULL)){
...
static struct of_device_id gfx1xm_of_match[] = {
	{ .compatible = "mediatek,mt6753-finger", },
	{}
};
