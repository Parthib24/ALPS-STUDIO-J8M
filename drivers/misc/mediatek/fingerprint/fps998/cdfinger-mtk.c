...
static int cdfinger_parse_dts(struct cdfinger_data *cdfinger)
{
...
#ifndef DTS_PROBE
	cdfinger->spi->dev.of_node = of_find_compatible_node(NULL,NULL,"mediatek,mt6735m-finger");
#endif
	...
	cdfinger->irq = irq_of_parse_and_map(finger_irq_node, 0);
	if(cdfinger->irq < 0)
	{
...
//		cdfinger->irq, cdfinger->vdd_ldo_enable, cdfinger->vio_ldo_enable, cdfinger->config_spi_pin);
	...
	cdfinger->cdfinger_irq = pinctrl_lookup_state(cdfinger->fps_pinctrl,"fingerprint_pin_irq");
...
static  int cdfinger_power_init(struct spi_device *spi) 
{
    int ret = 0;
...
{
	if(0x98 == chip_ID) {
		seq_printf(m, "fps998E\n" );
	}else{
		seq_printf(m, "Error!g\n" );
	}		
	return 0;
}
...
static ssize_t cdfinger_chip_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x", g_cdfinger->sensor_type);
}
...
static DEVICE_ATTR(chipId, S_IRUGO, cdfinger_chip_show, NULL);
