...
#define PFX "b50028_OV8856" 

#define RG_Ratio_Typical   0x14E
#define BG_Ratio_Typical   0x159
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = B50028_OV8856_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("[B50028_ov8856 get_imgsensor_id] sensor_id = %d \n",*sensor_id);
		if(main_sub != 1)
	{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            ...
            LOG_INF("ov8856:read sensor id success, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				if((read_cmos_sensor(0x302A)) == 0XB0){
 				ov8856version = OV8856R1A;
				LOG_INF("ov8856: i2c write id: 0x%x, sensor id: 0x%x, ov8856version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856version);	
				return ERROR_NONE;
				}
				//return ERROR_NONE;
				//}
				}
			LOG_INF("ov8856:Read sensor id fail, i2c_write_id: 0x%x sensor_id = %d \n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
        LOG_INF("ov8856:Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
UINT32 B50028_OV8856_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
