...
#include "subhi553mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "subHI553_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    	.sensor_id = SUBHI553_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
char Sub_Hynix_i2c_buf[HYNIX_SIZEOF_I2C_BUF];
static void Hi553_write_burst_mode(struct Hynix_Sensor_reg table[])
{
	...
	for (curr = table; curr->addr != HYNIX_TABLE_END; curr++) 
    {
		if (!buf_count) { 
			...
			Sub_Hynix_i2c_buf[buf_count] = curr->addr >> 8; buf_count++;
            
			Sub_Hynix_i2c_buf[buf_count] = curr->addr & 0xFF; buf_count++;

		}
        
		Sub_Hynix_i2c_buf[buf_count] = curr->val >> 8; buf_count++;

		Sub_Hynix_i2c_buf[buf_count] = curr->val & 0xFF; buf_count++;
        ...
        kdSetI2CSpeed(400);
        iBurstWriteReg(Sub_Hynix_i2c_buf , buf_count, imgsensor.i2c_write_id);
...
/*static void write_subhi553_sensor(kal_uint32 addr, kal_uint32 para)
{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	kal_uint8 i = 0;
	kal_uint8 retry = 4;
	printk("qingfeng subhi553 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
	if(main_sub != 2)
	{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
            ...
            *sensor_id = return_sensor_id()+1;
				if (*sensor_id == imgsensor_info.sensor_id) {				
                                        printk("subhi553 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					return ERROR_NONE;
				}	
				retry--;
			} while(retry > 0);
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            //LOG_INF("subhi553 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SUBHI553_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
