...
static kal_uint16 read_cmos_sensor_16(kal_uint32 addr)
{
	...
	printk("read_cmos_sensor_16(0x%x) temp_data[0]=0x%x, temp_data[1]=0x%x\n",addr,temp_data[0],temp_data[1]);
	return get_word;
}
#endif
...
{
	char pu_send_cmd[4] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para)};
	kdSetI2CSpeed(400); 
...
#ifdef Hi846_OTP_FUNCTION //Hi-846 OPT fuction begin
...
void HI846OTPSetting(void)
{
  ...
  HI846_OTP_write_cmos_sensor(0x070f, 0x05);//OTP data rewrite
...
void Hi846_Sensor_OTP_info(void)
{
  uint16_t ModuleHouseID = 0,
    ...
    uint16_t info_flag = 0,
       infocheck = 0,
       checksum = 0;
	//int i = 0, addr = 0;/*daizhiling add for read module info for test*/
  LOG_INF("%s enter\n",__func__);
...
void HI846_Sensor_calc_wbdata(void)
{
	//int i = 0, addr = 0;/*daizhiling add for read WB date for test*/
	uint16_t wbcheck = 0,
       checksum = 0,
       wb_flag = 0;
  ...
  //uint16_t temp_rg,temp_bg;
	...
	switch (wb_flag)
	{
		case 0x01:  //Group1 valid
		    ...
		    wb_unit_gr_h = HI846_Sensor_OTP_read(0x0c70);
		    wb_unit_gr_l = HI846_Sensor_OTP_read(0x0c71);
			wb_unit_gb_h = HI846_Sensor_OTP_read(0x0c72);
		    wb_unit_gb_l = HI846_Sensor_OTP_read(0x0c73);
			wb_golden_r_h = HI846_Sensor_OTP_read(0x0c74);
		    wb_golden_r_l = HI846_Sensor_OTP_read(0x0c75);
		    wb_golden_b_h = HI846_Sensor_OTP_read(0x0c76);
		    wb_golden_b_l = HI846_Sensor_OTP_read(0x0c77);
		    wb_golden_gr_h = HI846_Sensor_OTP_read(0x0c78);
		    wb_golden_gr_l = HI846_Sensor_OTP_read(0x0c79);
			wb_golden_gb_h = HI846_Sensor_OTP_read(0x0c7a);
		    wb_golden_gb_l = HI846_Sensor_OTP_read(0x0c7b);
		...
		case 0x02:  //Group2 valid
		    ...
		    wb_unit_r_h = HI846_Sensor_OTP_read(0x0c8a);
		    wb_unit_r_l = HI846_Sensor_OTP_read(0x0c8b);
		    wb_unit_b_h = HI846_Sensor_OTP_read(0x0c8c);
		    wb_unit_b_l = HI846_Sensor_OTP_read(0x0c8d);
		    wb_unit_gr_h = HI846_Sensor_OTP_read(0x0c8e);
		    wb_unit_gr_l = HI846_Sensor_OTP_read(0x0c8f);
			wb_unit_gb_h = HI846_Sensor_OTP_read(0x0c90);
		    wb_unit_gb_l = HI846_Sensor_OTP_read(0x0c91);
			wb_golden_r_h = HI846_Sensor_OTP_read(0x0c92);
		    wb_golden_r_l = HI846_Sensor_OTP_read(0x0c93);
		    wb_golden_b_h = HI846_Sensor_OTP_read(0x0c94);
		    wb_golden_b_l = HI846_Sensor_OTP_read(0x0c95);
		    wb_golden_gr_h = HI846_Sensor_OTP_read(0x0c96);
		    wb_golden_gr_l = HI846_Sensor_OTP_read(0x0c97);
			wb_golden_gb_h = HI846_Sensor_OTP_read(0x0c98);
		    wb_golden_gb_l = HI846_Sensor_OTP_read(0x0c99);
		...
		case 0x03:  //Group3 valid
		    ...
		    wb_unit_r_h = HI846_Sensor_OTP_read(0x0ca8);
		    wb_unit_r_l = HI846_Sensor_OTP_read(0x0ca9);
		    wb_unit_b_h = HI846_Sensor_OTP_read(0x0caa);
		    wb_unit_b_l = HI846_Sensor_OTP_read(0x0cab);
		    wb_unit_gr_h = HI846_Sensor_OTP_read(0x0cac);
		    wb_unit_gr_l = HI846_Sensor_OTP_read(0x0cad);
			wb_unit_gb_h = HI846_Sensor_OTP_read(0x0cae);
		    wb_unit_gb_l = HI846_Sensor_OTP_read(0x0caf);
			wb_golden_r_h = HI846_Sensor_OTP_read(0x0cb0);
		    wb_golden_r_l = HI846_Sensor_OTP_read(0x0cb1);
		    wb_golden_b_h = HI846_Sensor_OTP_read(0x0cb2);
		    wb_golden_b_l = HI846_Sensor_OTP_read(0x0cb3);
		    wb_golden_gr_h = HI846_Sensor_OTP_read(0x0cb4);
		    wb_golden_gr_l = HI846_Sensor_OTP_read(0x0cb5);
			wb_golden_gb_h = HI846_Sensor_OTP_read(0x0cb6);
		    wb_golden_gb_l = HI846_Sensor_OTP_read(0x0cb7);
...
//  wb_golden_bg_l = 0x09;//
    ...
    checksum = (wb_unit_rg_h + wb_unit_rg_l + wb_unit_bg_h + wb_unit_bg_l + wb_unit_gg_h + wb_unit_gg_l
      ...
      + wb_golden_r_h + wb_golden_r_l + wb_golden_b_h + wb_golden_b_l + wb_golden_gr_h + wb_golden_gr_l+ wb_golden_gb_h + wb_golden_gb_l ) % 255+1;

	//checksum = (wb_unit_rg_h + wb_unit_rg_l + wb_unit_bg_h + wb_unit_bg_l + wb_unit_gg_h + wb_unit_gg_l
	//	+ wb_golden_rg_h + wb_golden_rg_l + wb_golden_bg_h + wb_golden_bg_l + wb_golden_gg_h + wb_golden_gg_l) % 256;
	printk("wangk_HI846_Sensor: checksum = 0x%x,wbcheck= 0x%x\n ",checksum,wbcheck);
	...
	rg_golden_value = 0x11c;//((wb_golden_rg_h << 8)|wb_golden_rg_l);  //0x0278
	bg_golden_value = 0x15a;//((wb_golden_bg_h << 8)|wb_golden_bg_l);  //0x0252
...
{    
	...
	kal_uint8 retry = 2;
	//LOG_INF(" \n ");
	//LOG_INF("@@ hi846 get_imgsensor_id  i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
