...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = B50048_OV5695MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value =  0xc61fa88b,//0x523c51f6,		//checksum value for Camera Auto Test
...
int read_otp_ov5695(struct otp_struct *otp_ptr)
{
     int otp_flag,addr,temp,i;
     LOG_INF("@xiaofei_read_otp_ov5695_Start\n");
     //read otp info buffer
     ...
     write_cmos_sensor(0x3d81, 0x01);//load otp info buffer
     mDELAY(5);
     
     //otp WB calibration
     otp_flag=read_cmos_sensor(0x700c);
     addr=0;
	 LOG_INF("@xiaofei_ov5695 otp_flag=%x\n",read_cmos_sensor(0x700c)); 
	 LOG_INF("@xiaofei_ov5695 0x3d84=%x\n",read_cmos_sensor(0x3d84)); 
     ...
     if(addr != 0){
            ...
            LOG_INF("ov5695 module_integrator_id=%x\n",(*otp_ptr).module_integrator_id);
			LOG_INF("ov5695 lens_id=%x\n",(*otp_ptr).lens_id);
            LOG_INF("ov5695 rg_ratio=%x\n",(*otp_ptr).rg_ratio);
			LOG_INF("ov5695 bg_ratio=%x\n",(*otp_ptr).bg_ratio);   
     }
     else{
...
int apply_otp_ov5695(struct otp_struct *otp_ptr)
{
	  LOG_INF("@xiaofei_apply_otp_ov5695_Start\n");

      int RG_Ratio_Typical = 0x13A, BG_Ratio_Typical = 0x13A;  //???¨´?Y¨º¦Ì?¨º?¦Ì¨¬?D¡ä
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF(" ov5695 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	
...
static kal_uint32 open(void)
{
	...
	sensor_init();
		...
		read_otp_ov5695(otp_ptr);
		apply_otp_ov5695(otp_ptr);
		kfree(otp_ptr);
		write_cmos_sensor(0x0100, 0x00);
	#endif
...
UINT32 B50048_OV5695_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
