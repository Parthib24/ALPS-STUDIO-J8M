...
static int rgk_lcm_params_ssc_show(struct seq_file *m, void *v)
{
   seq_printf(m, "%s\n", (1 == lcm_ssc)?"off":"on");	
//   seq_printf(m, "%d\n", lcm_ssc);	
	return 0;
}	
static int rgk_lcm_params_ssc_open(struct inode *inode, struct file *file)
{
	return single_open(file, rgk_lcm_params_ssc_show, NULL);
}
static const struct file_operations rgk_lcm_params_ssc_proc_fops = {	
	.owner      = THIS_MODULE,
	.open       = rgk_lcm_params_ssc_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
	.release    = single_release,
};
  ...
  int rgk_creat_proc_lcm_params_info(void)
   {
		...
		struct proc_dir_entry *lcm_params_ssc_entry = proc_create("rgk_lcd_ssc_Info", 0444, NULL, &rgk_lcm_params_ssc_proc_fops);
	    if (lcm_params_ssc_entry == NULL)
		  pr_err("create /proc/lcm_ssc_entry fail\n");
	    return 0;
   }  
/*************add by wanghanfeng modify rgk_lcm_params_info_show *****end(2016-03-17)****************/
static int rgk_lcm_params_control_show(struct seq_file *m, void *v)
{
  seq_printf(m, "%d\n", lcm_clock);	
  return 0;
}
static int rgk_lcm_params_control_open(struct inode *inode, struct file *file)
{
  return single_open(file, rgk_lcm_params_control_show, NULL);
}
static int rgk_lcm_params_control_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
  int len = 0;
	char desc[32];
	int en,ret;	
	...
	else if (0 == strncmp(desc, "ssc_enable:", 11)) {
		char *p = (char *)desc + 11;
		//uint32_t clk = 0;
	...
	printk( "  bad argument, echo [enable] > lcm_clock\n");
	return -EINVAL;
}
static const struct file_operations rgk_lcm_params_control_proc_fops = {	
  .owner      = THIS_MODULE,
  .open       = rgk_lcm_params_control_open,
  .read       = seq_read,
  .write      = rgk_lcm_params_control_write,
  .llseek     = seq_lseek,
  .release    = single_release,
};
int rgk_creat_proc_lcm_params_control(void)
{
  struct proc_dir_entry *lcm_params_control_entry = proc_create("rgk_lcd_params_Control", 0777, NULL, &rgk_lcm_params_control_proc_fops);
  if (lcm_params_control_entry == NULL)
    pr_err("create /proc/lcm_control_entry fail\n");
  return 0;
}  
int ddp_dsi_init(DISP_MODULE_ENUM module, void *cmdq)
{
	...
	rgk_creat_proc_lcm_params_control();
	/*************add by wanghanfeng modify rgk_lcm_params_info_show *****end(2016-03-17)*****************/
...
void _dump_dsi_params(LCM_DSI_PARAMS *dsi_config)
{
	if (dsi_config) {
	 ...
	 /*************add by wanghanfeng modify rgk_lcm_params_info_show *****start(2016-03-17)****************/
	   ...
	   lcm_ssc   = dsi_config->ssc_disable;
     DISPDBG("@@@@@@@lcm_clock = %d@@@@@@@\n",lcm_clock);
	 DISPDBG("@@@@@@@lcm_ssc = %s@@@@@@@\n",(1 == lcm_ssc)?"off":"on");
	/*************add by wanghanfeng modify rgk_lcm_params_info_show *****end(2016-03-17)****************/
		DISPMSG
...
//	static cmdqBackupSlotHandle hSlot;
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			t0.Data_ID =
			    (t0.Data0 <
			     0xB0) ?  DSI_GERNERIC_READ_LONG_PACKET_ID:
		...
		if(packet_type == 0x1A || packet_type == 0x1C)
		   ...
		   printk("junjie packet_type");
            ...
            printk("esd check data: byte1.data0 =%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x,%x\n",\
      ...
      if((recv_data_cnt>=4) && 
         ...
         (read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2]) &&
			   ...
			   (read_data2.byte3 == dsi_params->lcm_esd_check_table[i].para_list[7]) &&
			   (read_data3.byte0 == dsi_params->lcm_esd_check_table[i].para_list[8]) &&
			   (read_data3.byte1 == dsi_params->lcm_esd_check_table[i].para_list[9]))
			{		
				...
				ret = 0; // esd pass
                                //break;
			}else if((recv_data_cnt==3) && (read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) && 
                           (read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1]) && 
                           (read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2]))
			{		
				...
				ret = 0; // esd pass
                        ...
                        else if((recv_data_cnt==2) && (read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) && 
                           (read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1]))
			{		
				...
				ret = 0; // esd pass
	                 ...
	                 else if((recv_data_cnt==1) && (read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]))
			{		
		...
		else
		 {
		     ...
		     printk("esd check data: %x,%x\n",read_data0.byte0,read_data0.byte1);
		    if(read_data0.byte1 == dsi_params->lcm_esd_check_table[i].para_list[0])
            //           if((recv_data_cnt==2) && (read_data0.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0]) && 
             //              (read_data1.byte1 == dsi_params->lcm_esd_check_table[i].para_list[1]))
			{		
	...
	else if(state == CMDQ_STOP_VDO_MODE)
	{
		...
		// -1. stop TE_RDY IRQ
		DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,TE_RDY,0);
