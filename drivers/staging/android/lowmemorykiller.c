...
//Add by amazon sop modify begin
#define FEATURE_OPT_AMAZON
#define MAX_DEPOSEPROC_NUM 12
typedef struct deposeProc{
   short prio;
   char procName[50];
}lmk_opt_depose_process_param_t;

#ifdef FEATURE_OPT_AMAZON
static lmk_opt_depose_process_param_t whiteList[MAX_DEPOSEPROC_NUM] = {
...
//Add by amazon sop modify begin
    ...
    short deposePrio = 0;
	//int deposeIndex = 0;
	struct task_struct *deposeSelected = NULL;
#endif
//Add by amazon sop modify end
...
//add by amazon sop modify begin
      ...
      if(deposeEnable > 0){
	  	int i = 0;
	    for(i=0;i<MAX_DEPOSEPROC_NUM;i++){
			lmk_opt_depose_process_param_t deposeProc = whiteList[i];
			//pr_err("mmmmmm_proc_name1:%s\n", deposeProc.procName); 
		    if(!strcmp(p->comm,deposeProc.procName))
				continue;
		}
      }else{
           int i = 0;
		  for(i=0; i<MAX_DEPOSEPROC_NUM; i++){
		  	lmk_opt_depose_process_param_t deposeProc = whiteList[i];
			//pr_err("mmmmmm_proc_name2:%s\n", deposeProc.procName); 
		  	if(!strcmp(p->comm,deposeProc.procName)){
				if(deposePrio < deposeProc.prio){
					deposePrio = deposeProc.prio;
					deposeSelected = p;
					}
				break;
		  		}
...
#ifdef FEATURE_OPT_AMAZON
   if(deposeSelected) {
   	  selected = deposeSelected;
   #ifndef CONFIG_MT_ENG_BUILD
   				selected_tasksize = get_mm_rss(deposeSelected->mm);
   #ifdef CONFIG_ZRAM
                selected_tasksize += get_mm_counter(deposeSelected->mm,MM_SWAPENTS);
   #endif
   #endif
   				selected_oom_score_adj = deposeSelected->signal->oom_score_adj;
   }
#endif
//add by amazon sop modify end
