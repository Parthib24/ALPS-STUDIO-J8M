...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		"[kernel]AvgVbat %4d,bat_vol %4d, AvgI %4d, I %4d, VChr %4d, AvgT %2d, T %2d, ZCV %4d, CHR_Type %1d, SOC %3d:%3d:%3d, bcct %1d:%1d, Ichg %6d, IBat %6d\n",
...
#ifdef RGK_MAX_CHARGE_TEMPERATURE_NOTIFY
	if (BMT_status.temperature >= RGK_MAX_CHARGE_TEMPERATURE_NOTIFY) {
#else
	if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
#endif
...
#ifdef BAT_LOW_TEMP_PROTECT_ENABLE
#ifdef RGK_MIN_CHARGE_TEMPERATURE_NOTIRY
	else if (BMT_status.temperature < RGK_MIN_CHARGE_TEMPERATURE_NOTIRY) {
#else
...
static void mt_battery_thermal_check(void)
{
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
#if defined(CUST_SHUTDOWN_TEMPERATURE)
    if (BMT_status.temperature >= CUST_SHUTDOWN_TEMPERATURE) {
#else
