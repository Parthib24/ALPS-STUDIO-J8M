...
#ifdef RLK_CUSTOM_CHARGE_SUPPORT
      if (g_bcct_flag != 1)
      {
        ...
        else if(BMT_status.temperature <= 55)
        {
          ...
          //g_temp_input_CC_value = CHARGE_CURRENT_1500_00_MA;
       	...
       	battery_log(BAT_LOG_CRTI, "[BATTERY] g_call_state: %d" , g_call_state);*/
      }
      /*if((g_bcct_flag == 1) && (g_lcm_state == 0))
      {
...
if (ta_check_ta_control == KAL_FALSE)
			...
			if (batt_cust_data.high_battery_voltage_support)
				...
				cv_voltage = BATTERY_VOLT_04_200000_V;
#ifdef CUST_CV_VOLTAGE
        cv_voltage = CUST_CV_VOLTAGE;
#endif
